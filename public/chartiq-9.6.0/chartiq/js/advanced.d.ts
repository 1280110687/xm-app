...
declare module '../js/chartiq.js' {
  export namespace CIQ.Drawing {
    ...
    class fibonacci {
       ...
       * @since 5.2.0
       */
      public recommendedLevels: number[]
      /**
       * Load default settings every time CIQ.ChartEngine#changeVectorType is called.
       * Will reset to  CIQ.Drawing.fibonacci#recommendedLevels
    ...
    class elliottwave {
       ...
       * @return Indicates whether the current wave data point has been positioned correctly.
       * @since 7.4.0
       */
  ...
  export namespace CIQ.Marker {
    ...
    class Performance {
       ...
       *
       * @param [params.size] Size class to add to your marker.
       ...
       * @param [params.displayLabel] Text to place in the marker.  Overrides `displayCategory`.
       ...
       * @param [params.hide] Do not process display of marker.
       * @param [params.enableLinks] If true, the story's content is trusted as safe HTML script.
       * 		As a result, any links in the story will be clickable.
       * @param [params.pinnable] If true, new marker can be "pinned". False by default.
       * 		Requires **js/extras/pinnedMarkers.js,** which is part of the ChartIQ Extras Package.
       * 		See CIQ.Marker.prototype.makePinnable for instructions on using this flag.
       *
       * @since
       * - 7.1.0
       * - 7.2.0 Markers without <u>both</u> a `headline` and `story` are not interactive.
       ...
       * 		style.
       * @param params.offset Height of the offset provided when creating the marker.
       * @param params.stemHeight Height of the marker stem offset as defined by the marker
       * 		stem CSS style height plus margin bottom.
       * @param params.inverted Indicates whether the marker stem is inverted; that is,
       * 		pointing downward.
       * @param params.field Field to attach marker to if using appropriate yPositioner.
       * @param params.x Already computed x position, in pixels.
  ...
  export namespace CIQ.Renderer {
     ...
     * - If you are customizing your Histogram to include a baseline, it is recommended that you render it on a separate panel,
     * especially if your primary series' renderer includes its own baseline. This is because histograms require their own
     * Y-Axis to render a baseline. The end result would include two distinct baselines, which might lead to confusion in data interpretation.
     * - While "stacked" histograms can be used with a combination of positive and negative values or with a baseline, the results are not optimal. Therefore, we do not recommend using these combinations, as they are not fully supported.
     *
     * If you want to display multiple series with baselines on the main chart,
     * refer to <a href="https://jsfiddle.net/8q13pm4u/2/" target="_blank">this example</a>
     * which demonstrates how to synchronize two distinct baselines.
     * **Note:** This advanced configuration is not included with ChartIQ and requires development on your end.
     *
     * @example
     ...
     * delete renderer;
     *
     * @example <caption>Create a histogram renderer on a separate panel with a baseline value, allowing negative bars.</caption>
     ...
     * @version ChartIQ Advanced Package
     * @since
     * - 7.5.0 Added the ability to draw negative bars when `yAxis.baseline` is set to zero
     * 		or some other value (see examples).
     * - 9.6.0 Added `config.params.priority` and `config.params.colMargin` parameter
     */
    class Histogram {
      /**
       * Creates a multi-part histogram renderer where bars can be stacked one on top of the other, clustered next to each other, or overlaid over each other.
       ...
       * @param  [config.params.colMargin] Space between columns in the cluster. Larger or equal to 1 defines pixel width between clustered columns, if less than 1 - percentage of the cluster width.
       ...
       * @param  [config.params.priority] Priority in the list of renderer layering order within over or under chart grouping. Renderers with higher priority will be
       * 		layered first. The parameter allows to render histograms with larger areas behind renderers with smaller areas.
       * 		It is applicable to all renderers.
       *
       * Common valid parameters for use by attachSeries.:
       * `fill_color_up` - Color for histogram bars when the Close is *higher* than the previous Close.
       * `fill_color_down` - Color for histogram bars when the Close is *lower* than the previous Close.
       * `border_color_up` - Border color for histogram bars when the Close is *higher* than the previous Close.
       * `border_color_down` - Border color for histogram bars when the Close is *lower* than the previous Close.
       *
       * See CIQ.Renderer#attachSeries for additional parameters.
       *
       * @example
       * // configure the histogram display
       * var params={
    ...
    class Heatmap {
       ...
       * @param  [config.params.height] The amount of vertical space to use, in price units. For example, 2=>2 unit increments on y-axis.
       * @version ChartIQ Advanced Package
       * @example
       *  // note special data formatting, where the custom field name that will be used for the stream of datapoints, is an array of values.
       *  var renderer=stxx.setSeriesRenderer(new CIQ.Renderer.Heatmap());
  ...
  export namespace CIQ {
     ...
     * @param stx   The chart object
     * @param newData The data to aggregate. Normally the dataSet.
     * @param range The price range for the range bars. This is typically user configurable. Defaults to a range size so that about 300 bars worth of time are displayed, about a year for a daily chart, and about 5 hours on a minute chart.
     * @param computed Cumulative computed records from last pass through this function. Used to increase performance by reusing pre-calculated bars and only calculate missing new bars.
     * @return        The aggregated data
  ...
  export namespace CIQ.Studies {
     ...
     * @param quotes the dataSegment
     */
    function calculateValuationLines(
      stx: CIQ.ChartEngine,
     ...
     */
    function calculateCorrelationCoefficient(stx: CIQ.ChartEngine, sd: object): void
    /**
     * Calculate function for klinger
     * @param  stx Chart object
     * @param sd  Study Descriptor
     */
    function calculateKlinger(stx: CIQ.ChartEngine, sd: CIQ.Studies.StudyDescriptor): void
    /**
     ...
     * @param sd  Study Descriptor
     */
    function calculateMACD(stx: CIQ.ChartEngine, sd: CIQ.Studies.StudyDescriptor): void
    /**
     ...
     * allowed by CIQ.ChartEngine#maxDataSetSize, which breaks the study. If you have set
     ...
     *
     * @param stx A reference to the chart object.
     * @param sd Specifies the study (PVAT or PAV) for which the lookback is
     * 		validated.
     * @return True if the the lookback is valid; otherwise, false.
     *
     * @since 8.0.0
     */
    function validateProjectedVolumeLookback(stx: CIQ.ChartEngine, sd: CIQ.Studies.StudyDescriptor): boolean
    /**
     ...
     *
     * @param stx A reference to the chart object.
     * @param sd Specifies the study (PVAT or PAV) for which the projected
     * 		volume is calculated.
     *
     * @since 8.0.0
     */
    function calculateProjectedVolume(stx: CIQ.ChartEngine, sd: CIQ.Studies.StudyDescriptor): void
    /**
     * Displays the Projected Volume at Time (PVAT) and Projected Aggregate Volume (PAV) studies.
     *
     * @param stx A reference to the chart object.
     * @param sd Specifies the study (PVAT or PAV) to be displayed.
     * @param quotes An array of quotes from which the study is constructed.
     *
     * @since 8.0.0
     */
    function displayProjectedVolume(
      stx: CIQ.ChartEngine,
      sd: CIQ.Studies.StudyDescriptor,
      quotes: object[]
    ): void
     ...
     */
    function calculateRSI(stx: CIQ.ChartEngine, sd: CIQ.Studies.StudyDescriptor): void
    /**
     * Calculate function for stochastics
     * @param  stx Chart object
     * @param sd  Study Descriptor
     */
    function calculateStochastics(stx: CIQ.ChartEngine, sd: CIQ.Studies.StudyDescriptor): void
    /**
     ...
     */
    function displayValuationLines(stx: CIQ.ChartEngine, sd: CIQ.Studies.StudyDescriptor): void
  }
}
export function aggregations(_export: object): void
export function drawingAdvanced(_export: object): void
export function equationsAdvanced(_export: object): void
export function highPerformanceMarkers(_export: object): void
export function renderersAdvanced(_export: object): void
export function typedefs(_export: object): void
export function accumulationDistribution(_export: object): void
export function adx(_export: object): void
export function alligator(_export: object): void
export function aroon(_export: object): void
export function atr(_export: object): void
export function awesomeOscillator(_export: object): void
export function balanceOfPower(_export: object): void
export function bollinger(_export: object): void
export function cci(_export: object): void
export function centerOfGravity(_export: object): void
export function chaikin(_export: object): void
export function chande(_export: object): void
export function choppiness(_export: object): void
export function comparisonStudies(_export: object): void
export function coppock(_export: object): void
export function darvasBox(_export: object): void
export function detrended(_export: object): void
export function disparity(_export: object): void
export function easeOfMovement(_export: object): void
export function ehlerFisher(_export: object): void
export function elder(_export: object): void
export function fractalChaos(_export: object): void
export function goNoGo(_export: object): void
export function highLowStudies(_export: object): void
export function ichimoku(_export: object): void
export function intradayMomentum(_export: object): void
export function keltner(_export: object): void
export function klinger(_export: object): void
export function linearRegression(_export: object): void
export function macd(_export: object): void
export function massIndex(_export: object): void
export function moneyFlow(_export: object): void
export function movingAverages(_export: object): void
export function parabolicSAR(_export: object): void
export function pivotPoints(_export: object): void
export function prettyGoodOscillator(_export: object): void
export function priceMomentumOscillator(_export: object): void
export function priceVolumeOscillator(_export: object): void
export function primeNumber(_export: object): void
export function pring(_export: object): void
export function projectedVolume(_export: object): void
export function psychologicalLine(_export: object): void
export function qstick(_export: object): void
export function rainbow(_export: object): void
export function randomWalk(_export: object): void
export function relativeVigor(_export: object): void
export function rsi(_export: object): void
export function schaffTrendCycle(_export: object): void
export function shinohara(_export: object): void
export function stochastics(_export: object): void
export function supertrend(_export: object): void
export function swingIndex(_export: object): void
export function trendIntensity(_export: object): void
export function trix(_export: object): void
export function twiggsMoneyFlow(_export: object): void
export function typicalPrice(_export: object): void
export function ulcerIndex(_export: object): void
export function ultimateOscillator(_export: object): void
export function valuationLines(_export: object): void
export function volatilityIndex(_export: object): void
export function volumeProfile(_export: object): void
export function volumeStudies(_export: object): void
export function vortex(_export: object): void
export function williamsMFI(_export: object): void