 ...
 * @param {boolean} [params.stretchGaps] Set to true to cause gaps to be represented by a horizontal line.
 * @param {number} [params.pixelWidth] Amount of width that the candle drawing takes, slightly less than candleWidth. Defaults to 65% of candleWidth.
...
CIQ.ChartEngine.prototype.drawHLCBox = function (params) {
	var shaded = params.shaded,
		...
		stretchGaps = params.stretchGaps,
	...
	if (!stretchGaps && stretchGaps !== false)
		stretchGaps = this.cleanupGaps === "stretch";
	...
	if (!pixelWidth) pixelWidth = candleWidth * 0.65;
	var xbase = panel.left - 0.5 * candleWidth + this.micropixels - 1;
	var defaultWhitespace =
		(widthFactor ? candleWidth * widthFactor : pixelWidth) / 2; //for each side of the candle
	var x0 = -1;
	for (var x = 0; x <= quotes.length; x++) {
		xbase += (this.layout.candleWidth * (x - x0 - 1)) / 2;
		x0 = x;
		...
		if (quote.candleWidth) {
			...
			else if (candleWidth < pixelWidth) whitespace = candleWidth / 2;
		}
		var transformFunc = this.getTransformFunction(yAxis);
		var transformedQuote = this.getTransformedQuote(quote, yAxis);
		if (transformFunc && transformedQuote) quote = transformedQuote;
		if (quote && field) quote = quote[field];
		if (!quote && quote !== 0) continue;
		var Close = quote[closeField];
		if (stretchGaps) {
			while (quotes[x + 1]) {
				var nextQuote = quotes[x + 1].Close;
				if (nextQuote || nextQuote === 0 || quotes[x + 1].futureTick) break;
				xbase += candleWidth / 2;
				whitespace += candleWidth / 2;
				x++;
			}
		}
