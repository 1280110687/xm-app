 ...
 * @since 8.9.0
 */
CIQ.MobileBridge.setRefreshInterval = function (newInterval) {
	stxx.quoteDriver.resetRefreshInterval(newInterval);
};
/**
 ...
 * @param {boolean} [moreAvailable] Set this to false to stop pagination requests if you know that no older data is available.
 * @param {boolean} [upToDate] Set this to true to stop forward pagination requests if you know that no newer data is available.
 * @param {boolean} [useAsLastSale] Set this parameter to true to force the data as a last sale price.
 *
 * @memberof CIQ.MobileBridge
 */
CIQ.MobileBridge.parseData = function (
	data,
	callbackId,
	moreAvailable,
	upToDate,
	...
	for (var i = 0; i < data.length; i++) {
		newQuotes[i] = {};
		newQuotes[i].DT = new Date(data[i].DT);
	...
	if (callbackId) {
		...
		if (typeof upToDate === "undefined") upToDate = false;
		...
		// push method
		if (typeof useAsLastSale === "undefined") useAsLastSale = false;
		stxx.updateChartData(newQuotes, null, { useAsLastSale: useAsLastSale });
	}
};
/**
 ...
 * @param {string} [data] Stringified JSON object of your data.
 *
 * @memberof CIQ.MobileBridge
 *
 * @example
 ...
 *     chartIQView.push(symbol, pushData)
...
CIQ.MobileBridge.getAllSeries = function () {
	...
	for (const key in allSeries) {
		if (allSeries[key].parameters.action === "add-study") continue; // don't send back the series being used by studies
		seriesObj[key] = {};
...
CIQ.MobileBridge.getDrawingParameters = function (toolName) {
	...
	if (
		...
		stxx.currentVectorParameters.color1 = "auto";
		stxx.currentVectorParameters.color2 = "auto";
		stxx.currentVectorParameters.color3 = "auto";
	}
	if (
...
CIQ.MobileBridge.setDrawingParameters = function (parameterName, value) {
	...
	switch (parameterName) {
		...
		case "fibs":
			...
			break;
		case "priceBuckets":
			stxx.currentVectorParameters.volumeProfile.priceBuckets = value;
			break;
		case "volumeProfile":
			let obj = JSON.parse(value);
			stxx.currentVectorParameters.volumeProfile.priceBuckets =
				obj.priceBuckets;
			break;
		default:
 ...
 *
 * @param {String} studyName The name of the study to add to the chart.
 *
 * @return {string} JSON string of a slimmed down version of the study descriptor.
 *
 * @memberof CIQ.MobileBridge
 * @since 8.8.0
 */
CIQ.MobileBridge.addStudyAsSignal = function (studyName) {
	const sd = CIQ.Studies.addStudy(stxx, studyName);
	const slimSd = this.getSlimSd(sd.name);
	return slimSd;
};
/**
 * Helper function to remove the signal from the chart.
 *
 * @param {String} studyName The name of the study to remove from the chart.
 *
 * @memberof CIQ.MobileBridge
 * @since 8.8.0
 */
CIQ.MobileBridge.removeSignal = function (studyName) {
	this.removeStudy(studyName);
};
/**
 * Helper function to save the signal parameters and make the signal available to display.
 *
 * @param {String} studyName The name of the study to add to the chart.
 * @param {String} signalParams JSON string that represents the signal parameters object.
 * @param {boolean} editMode Determines whether signal is in edit or create mode.
 *
 * @memberof CIQ.MobileBridge
 * @since 8.8.0
 */
CIQ.MobileBridge.saveSignal = function (studyName, signalParams, editMode) {
	// find active study
	const sd = stxx.layout.studies[studyName];
	// parse signalParams
	try {
		const parsedParams = JSON.parse(signalParams);
		// name and description are encoded to handle special characters, time to decode
		parsedParams.name = decodeURIComponent(parsedParams.name);
		parsedParams.description = decodeURIComponent(parsedParams.description);
		// save the signal
		stxx.signalIQ.convertStudyToSignal(sd, parsedParams, editMode);
	} catch (error) {
		console.log(error);
	}
};
 ...
 * @since 8.8.0
 */
CIQ.MobileBridge.toggleSignalStudy = function (studyName) {
	const sd = stxx.layout.studies[studyName];
	if (sd) {
		sd.toggleDisabledState(stxx);
	}
};
/**
 ...
 * @since 8.8.0
 */
CIQ.MobileBridge.getActiveSignals = function () {
	var results = [];
	for (let key in stxx.layout.studies) {
		var slimSd = {};
		var study = stxx.layout.studies[key];
		if (study.signalData) {
			slimSd.conditions = study.signalData.conditions;
			slimSd.description = study.signalData.description;
			if (this.isAndroid)
				slimSd.description = encodeURIComponent(slimSd.description); // Android needs to be encoded or the special characters are not guaranteed to copy over, iOS works fine.
			slimSd.joiner = study.signalData.joiner;
			slimSd.signalName = study.signalData.name;
			if (this.isAndroid)
				slimSd.signalName = encodeURIComponent(slimSd.signalName); // Android needs to be encoded or the special characters are not guaranteed to copy over, iOS works fine.
			slimSd.studyName = study.name;
			slimSd.disabled = study.disabled;
			slimSd.outputs = study.outputMap;
			slimSd.type = study.type;
			slimSd.uniqueId = study.uniqueId;
			results.push(slimSd);
		}
	}
 ...
 * @since 8.8.0
 */
CIQ.MobileBridge.getSlimSd = function (studyName) {
	const sd = stxx.layout.studies[studyName];
	let slimSd = {};
	slimSd.outputs = sd.outputMap;
	slimSd.studyName = sd.name;
	slimSd.type = sd.type;
	slimSd.uniqueId = sd.uniqueId;
	return JSON.stringify(slimSd);
};
//////////////////////////
/*** Study functions ***/
/////////////////////////
...
CIQ.MobileBridge.addStudy = function (studyName, inputs, outputs, parameters) {
	...
	for (var i in inputs) {
		// set any arrays to the default (the first item in the array)
...
/**
 * Returns an JSON stringified object of all the studies in the {@link CIQ.Studies.studyLibrary} with a short name
 * derived from the key.
 *
 * Used for gathering all available studies a user can access.
 *
 * @return {String} JSON stringified array of studies with the short name of study.
 *
 * @memberof CIQ.MobileBridge
 */
CIQ.MobileBridge.getStudyList = function () {
	let studies = CIQ.Studies.studyLibrary;
	// AVWAP doesn't currently work on the native apps for SignalIQ
	if (stxx.signalIQ) {
		let allowedStudies = stxx.signalIQ.allowedStudies.filter(
			(study) => !["AVWAP"].includes(study)
		);
		for (const key in studies) {
			// check if study is allowed for SignalIQ
			if (!allowedStudies.includes(key)) {
				studies[key].signalIQExclude = true;
			}
		}
	}
