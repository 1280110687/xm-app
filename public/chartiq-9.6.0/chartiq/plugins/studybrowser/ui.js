...
if (!CIQ.UI) {
	console.error("Study Browser requires importing componentUI.js.");
} else {
	/**
	 *  @classdesc
	 *
	...
	class StudyBrowser extends CIQ.UI.ContextTag {
		...
		constructor() {
			 ...
			 *
			 * @type {object}
			 * @memberof WebComponents.StudyBrowser#
			 * @alias interactiveStudies
			 * @instance
			 */
		 ...
		 *
		 * @tsmember WebComponents.StudyBrowser
		 */
		setContext(context) {
			if (this.init) return;
		...
		hide() {
			...
			if (this.parentMenu) {
				if (this.parentMenu.tabIndex === undefined)
					this.parentMenu.tabIndex = -1;
		 ...
		 *
		 * @param {string} filterName Name of category to show
		 * @param {boolean} [force] If true forces a refresh even if new filter is same as current filter.
		 * @tsmember WebComponents.StudyBrowser
		 */
		 ...
		 *
		 * @param {string} type Left panel menu selection
		 * @tsmember WebComponents.StudyBrowser
		 */
		selectType(type) {
			if (type === undefined)
				type = this.getAttribute("ciq-study-type") || "all";
		...
		setupInfoPanel() {
			...
			this.makeTap(this.querySelector(".ciq-info-panel"), (e) => {
				...
				if (
					!(addingStudy && this.interactiveStudies[this.infoPanelStudy.name])
				) {
					e.stopPropagation();
				}
			});
		 ...
		 *
		 * @param {boolean} studyType Study type
		 * @tsmember WebComponents.StudyBrowser
		 */
