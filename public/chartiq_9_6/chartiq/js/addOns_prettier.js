        ...
        if (styles.session) {
          ...
          for (i = 0; i < ranges.length; i++) {
            ...
            if (!noDashes && ranges[i].end < chart.right)
              this.plotLine(
                CIQ.extend(
                  { x0: ranges[i].end, x1: ranges[i].end },
                  dividerStyle,
                ),
              );
          }
    ...
    function (params) {
      ...
      this.wakeChart = function (isPropagated) {
        ...
        if (this.sleeping) {
          ...
          this.sleeping = false;
          this.stx.container.ownerDocument.body.classList.remove(
            "ciq-sleeping",
          );
          if (!isPropagated) {
  ...
  CIQ.RangeSlider =
    ...
    function (params) {
      ...
      this.acceptLayoutChange = function (layout) {
        var doDraw = false;
        if (stx.layout.rangeSlider !== layout.rangeSlider) {
      ...
      this.drawSlider = function () {
        ...
        var pLeft = (self.pixelLeft =
          ...
          (segmentImage[left]
            ? segmentImage[left].candleWidth / 2
            : halfCandle));
          ...
          function (e) {
            if (!self.timeout) {
              ...
              self.timeout = setTimeout(() => {
                ...
                let newCandleWidth =
                  stx.chart.width / (tickRight - tickLeft + 1);
                if (
                  tickRight >= tickLeft &&
...
let __js_addons_standard_tableView_ = (_exports) => {
  ...
  CIQ.TableView.prototype.subscribeToChanges = function (
    ...
    const { channels: { tableView = channelPath } = {} } =
      uiContext.config || {};
    const { stx } = this;
  ...
  TableViewBuilder.createToolbar = function (stx, config = {}) {
    ...
    const toolbar = getCoverToolbar({
      stx,
      symbol: symbolDisplay || display || symbol,
      copyFn,
      downloadFn,
  ...
  TableViewBuilder.getColumnHeaders = function (stx) {
    ...
    if (!stx.chart.highLowBars || !stx.chart.symbol) {
      delete colHeaders.open;
      delete colHeaders.high;
      delete colHeaders.low;
    }

    if (!stx.chart.symbol) delete colHeaders.close;

    if (!stx.tableView.viewAdditionalColumns || !stx.chart.symbol) {
  ...
  TableViewBuilder.createTable = function (stx, config = {}) {
    ...
    tableView.toolbar.symbol.textContent =
      symbolDisplay ||
      display ||
      symbol ||
      Object.keys(stx.chart.series).join(", ");

    tableView.toolbar.symbol.classList.toggle(
      "ciq-secondary",
      !stx.chart.symbol,
    );
    tableView.toolbar.additional.style.display = stx.chart.symbol ? "" : "none";
    ...
    setTimeout(() => {
      ...
      if (scrollbarStyling) {
        ...
        scrollbarStyling.refresh(
          cover.querySelector(".ciq-data-table-wrapper"),
          {
   ...
   * @since 8.1.0
   */
  TableViewBuilder.dataToHtml = function (
    data,
    { colHeaders, minColumnWidth },
  ) {
    const keyLength = Object.keys(colHeaders).length;
    const colWidth = `calc((100% - ${10 + keyLength * 4}px ) / ${keyLength})`;
    const tableHeader = Object.entries(colHeaders).map(
      ([, { label }], index) => {
        return `<th
			scope="col"
  ...
  TableViewBuilder.getValueFormatter = function (stx, decimalPlaces) {
    ...
    if (yAxis.originalPriceFormatter && yAxis.originalPriceFormatter.func) {
      ...
      formatValue = (value) =>
        stx.formatYAxisPrice(value, panel, decimalPlaces);
    }
  ...
  TableViewBuilder.getFilenameFormatter = function (stx) {
    return (csvData) => {
      const symbol =
        stx.chart.symbolDisplay ||
        stx.chart.symbolObject.display ||
        stx.chart.symbol ||
        Object.keys(stx.chart.series).join("_");
    ...
    CIQ.Tooltip =
      ...
      function (tooltipParams) {
        ...
        CIQ.Marker.Tooltip.placementFunction = function (params) {
          ...
          for (var i = 0; i < params.arr.length; i++) {
            ...
            if (
              ...
              marker.node.style.right =
                Math.round(currentBarOffsetRight) + "px";
            }
        ...
        function hideTooltip() {
          ...
          huTooltip.lastBar = {};
          if (huTooltip.highlightEl)
            huTooltip.highlightEl.style.display = "none";
        }
        ...
        function renderFunction() {
          ...
          if (!stx.chart.symbol)
            node.parentElement
              .querySelectorAll("[hu-tooltip-field=Close]")
          ...
          if (showSeries) {
            ...
            for (var renderer in renderers) {
              ...
              for (
                var id = 0;
                id < rendererToDisplay.seriesParams.length;
                id++
              ) {
          ...
          for (var f = 0; f < fields.length; f++) {
            ...
            var nameField =
              name === "Close" && data._periodic_Close
                ? "_periodic_Close"
                : name;
            var tuple = CIQ.existsInObjectChain(data, nameField);
            ...
            if (
              ...
              (isRecordDate ||
                ...
                dsField.Close ||
                dsField.Close === 0 ||
                dsField._periodic_Close ||
                dsField._periodic_Close === 0)
            ) {
              var fieldValue = "";
              if (dsField.Close || dsField.Close === 0) dsField = dsField.Close;
              if (dsField._periodic_Close || dsField._periodic_Close === 0)
                dsField = dsField._periodic_Close;
              ...
              if (naField) {
                var naFieldNameField =
                  naField.querySelector(".hu-tooltip-name");
...
let __js_addons_advanced_animation_ = (_exports) => {
  ...
  CIQ.Animation =
    ...
    function (config) {
      ...
      stx.tickAnimator =
        easeMachine || new CIQ.EaseMachine("easeOutCubic", 1000);
      var scrollAnimator = new CIQ.EaseMachine("easeInOutCubic", 1000);

      var flashingColors = ["#F7931A", "#F7931A20", "#F7931A", "#F7931A20"];
      ...
      stx.prepend("updateChartData", function (appendQuotes, chart, params) {
        ...
        var supportedChartType = this.mainSeriesRenderer.supportsAnimation;
        ...
        if (supportedChartType) {
          ...
          if (interval == "day" || timeUnit == "day")
            chartJustAdvanced =
              quote.DT.getDate() != this.prevQuote.DT.getDate();
          ...
          var linearChart = !this.mainSeriesRenderer.standaloneBars;
              ...
              if (chart.scroll <= chart.maxTicks) {
                ...
                if (
                  chart.dataSegment.length <
                    chart.maxTicks -
                      animationParameters.ticksFromEdgeOfScreen &&
      ...
      stx.prepend("renderYAxis", function (chart) {
        ...
        if (scrollAnimator.running) scrollAnimator.stop(stx.ownerWindow);
...
let __js_addons_advanced_continuousZoom_ = (_exports) => {
  ...
  CIQ.ContinuousZoom.prototype.execute = function (zoomOut) {
    ...
    if (
      ...
      (!zoomOut &&
        boundaries.minTicks &&
        chart.maxTicks < boundaries.minTicks) ||
        ...
        Object.keys(this.markers).forEach(
          function (key) {
            if (
              ...
              this.markers[key].marker.node.classList.contains(
                this.displayState,
              )
            )
          ...
          function (targetAxis, key) {
            ...
            if (targetType == "quote") {
              if (targetValue > targetAxis.trendMax) {
                ...
                // Low marker
                if (targetAxis.isFlipped)
                  targetMarker.params.yPositioner = "top";
      ...
      function addSeries(stx, symbol, parameters, id) {
        function verifyQuoteFeed(stx) {
          ...
          for (var qf = 0; qf < stx.quoteDriver.quoteFeeds.length; qf++) {
            if (stx.quoteDriver.quoteFeeds[qf].engine == params.quoteFeed)
              return;
          }
          return "err";
        }
