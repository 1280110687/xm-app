 ...
 *
 * @param id An ID tag for a valid DOM element.
 * @param [source] A valid DOM node to search within. If not provided, the entire
 * 		document is searched.
 * @return The DOM node associated with the ID, or null if a node is not found.
 *
 */
export function $$(id: string, source?: HTMLElement): HTMLElement
 ...
 * @param [source] Node to select within. If not provided, the entire document is
 * 		searched.
 * @return The first element to match the selector.
 *
 */
export function $$$(selector: string, source?: HTMLElement): HTMLElement
...
export namespace CIQ.ChartEngine {
   ...
   * All quotes must at least have either a DT or Date property that is a JavaScript Date in order to be valid, every other value is nullable.
  ...
  /**
   * Append quote type for the [updateChartData]CIQ.ChartEngine#updateChartData method
   *
   */
  interface LastSale {
    /**
    ...
    Ask?: number
    /**
     * Level 2 Bid, expressed as an array of [price,size,obj] pairs.  <br>For example, BidL2: [[10.05, 15, {...}],[10.06, 10, {...}],...].<br>
    ...
    BidL2?: any[]
    /**
     * Level 2 Offer/Ask expressed as an array of [price,size,obj] pairs.  <br>For example, AskL2: [[11.05, 12, {...}],[11.06, 8, {...}],...].<br>
     * `obj` is an optional object which can contain whatever you wish.  It will be conveyed all the way into the marketdepth chart and can be displayed by using the 'headsUp' method of displaying crosshair data.
     */
    AskL2?: any[]
    /**
   ...
   *
   * @callback CIQ.ChartEngine~destroyEventListener
   * @since 8.8.0
   *
   * @see CIQ.ChartEngine#addEventListener
   */
  type destroyEventListener = (data: {stx: CIQ.ChartEngine}) => void
   ...
   *
   * @param data Data relevant to the "drawingComplete" event.
   * @param data.stx The chart engine instance.
   * @param data.drawing The completed drawing.
   *
   * @callback CIQ.ChartEngine~drawingCompleteEventListener
   * @since 8.6.0
   */
  type drawingCompleteEventListener = (data: {stx: CIQ.ChartEngine,drawing: CIQ.Drawing}) => void
   ...
   * 		WebComponents.FloatingWindow.DocWindow#positionRelativeTo). Defaults
   * 		to `document.body`.
   ...
   * - Using the WebComponents.DrawingSettings to disable the current active drawing tool
   *   or toggling the crosshair
   * - Using the WebComponents.Views to activate a serialized layout
   ...
   * 		Overrides the `default-position` attribute of the
   ...
   * 		Overrides the `default-transition` attribute of the
   * 		[`<cq-message-toaster>`]WebComponents.MessageToaster element.
   ...
   * 		dismissed. Overrides the `default-display-time` attribute of the
   * 		[`<cq-message-toaster>`]WebComponents.MessageToaster element.
   ...
   * WebComponents.Themes initialization or using the
   * WebComponents.ThemeDialog.
   *
  ...
  class Chart {
    ...
    /**
     * How much to multiply the [yAxis.initialMarginTop]CIQ.ChartEngine.YAxis#initialMarginTop and [yAxis.initialMarginBottom]CIQ.ChartEngine.YAxis#initialMarginTop for the main chart. Set to `1` to keep the initial values.
     * @since 8.6.0
     */
    public yaxisMarginMultiplier: number
    /**
  ...
  class Panel {
    ...
    /**
     * If set renders grid using the gridYAxisName y-axis rather than yAxis defined axis
     *
     * @since 9.6.0
     */
    public gridYAxisName: string
    /**
     ...
     * @since 9.2.0
     */
    public clipVerticalTextToPanel(text?: string, context?: object): string
  }
  ...
  /**
   * This is the object type stored in the "axisRepresentation" array, which is returned by CIQ.ChartEngine.createXAxis and is used to render the x-axis in CIQ.ChartEngine.drawXAxis.
  ...
  class XAxisLabel {
     ...
     * @param [params.priorDT] DT field from prior axisRepresentation record (prior label).
     * @since
     * - 8.4.0 Added raw and priority parameter, rationalized to object parameter.
     * - 8.8.0 Added `params.priorDT` parameter.
     */
  ...
  class YAxis {
    ...
    public name: string
    /**
     * If set, will override the axis title (if enabled)
     * Set to a function will receive chart engine, panel and y-axis as parameters to return the title string to display
     * @since 9.2.0
     */
    public title: string | Function | null
    public high: number
    public low: number
    ...
    /**
     * Clamped axis [min,max] values. If set, the axis will be clamped to these values. Do not set directly, use the axis `setClamp` method instead.
     * @since 9.5.0
     */
    public clamp: object
    /**
     ...
     * **Note:** `textStyle` only accepts hex or rgba color values.
     * @since 15-07-01
     */
    ...
    /**
     * Method to clamp axis to min/max values. If this function is called without a prefs parameter, the axis clamp will be removed.
     * Optionally, axis ticks can be fixed to a specific count. The count represents the number of ticks to
     * display between the clamped values.
     *
     * Call stx.draw() after setting clamp to update the chart.
     *
     * @param prefs Object containing the desired number of ticks and padding.
     * @param prefs.min minimum value for the axis. This will not be used unless prefs.max is set.
     * @param prefs.max maximum value for the axis. This will not be used unless prefs.min is set.
     * @param prefs.count Number of ticks to display. Number of ticks to display. Null resets count. Requires min and max to be set.
     * @param prefs.padding Distance in pixels between the min and max values and the top and bottom of the axis. Null resets padding to 0. Requires min, max and count to be set.
     * @since 9.5.0
     */
    public setClamp(
      prefs: {
     ...
     * to relative values. For instance, the net change axis feature uses a transform that adjusts from price to relative change over time.
     * After this is called, yAxis.transformFunc and yAxis.untransformFunc will be set to those functions.
     *
     * @param stx 		 The chart to remove transformation data from
     * @param name					 A unique name identifying the transformation
     ...
     * @since 9.6.0
     */
    public unsetTransform(stx: CIQ.ChartEngine): void
  }

  /**
   * When set to true, the scrubbed array is the same as the dataSet, making the series data available
   * for studies, even when there is no primary data for that data point.
   * @since 9.6.0
   */
  let doNotScrub: boolean
   ...
   *
   * @param quote An item from a quote feed
   * @param field The field key to extract from the quote feed object
   * @param [subField="Close"] The sub field to retrieve if the field value is an object
   * @return The value of the specified field in the quote object
   * @static
   * @since 9.4.0 Moved to CIQ.ChartEngine namespace.
   */
  function getQuoteFieldValue(quote: object, field: string, subField?: string): (number|null)
   ...
   * document.addEventListener(...).
   *
   * @param events Event or array of events for the document to listen for
   * @param listener Function to call when event is fired
   * @param [options] Options to pass to the event listener, e.g. `{passive:false}`
   * @param [isWindow] If true, attach to window, otherwise, to the document
   *
   * @static
   *
   * @since 8.5.0
  ...
  /**
   * Attaches events which were registed by CIQ.ChartEngine.registerGlobalEvent
   ...
   *
   */
  function attachGlobalEventHandlers(): void
...
export namespace CIQ {
  ...
  class ChartEngine {
    ...
    /**
     * Preserves candle width when resizing the chart canvas. Default resizes candles to preserve the x-axis range. Set to true to maintain candle size.
     * In a multi-chart environment, this value is automatically set to true, maintaining consistency in candle sizes. Set to false to resize candles in multi-chart grids.
     *
     * @since 8.8.0
     */
    public preserveCandleWidthOnResize: boolean
    /**
    ...
    /**
     * An array of marker types that can be grouped, rather than stacked, when positioned over the same tick. Currently defaults only to "Simple Circle", "Simple Square",
     * and high performance markers.
     *
     * Though any marker type can be grouped, using any type other than the default may not appear correctly in the listing when a group
     * marker is clicked.
     * @since 9.2.0
     * @example
     * stxx.groupableMarkerTypes=["circle", "square", "diamond", "text", "singleEvent"];
     */
    public groupableMarkerTypes: string[]
    /**
    ...
    public layout: {
       ...
       *  - "vertex_" - add to line, step, mountain, baseline_delta, and baseline_noncentered
       ...
       *
       */
      studies: Record<string, CIQ.Studies.StudyDescriptor>,
      /**
    ...
    public preferences: {
      ...
      /**
       * Determines whether the crosshairs automatically display when a user actives
       * a drawing tool. Can be set in the default chart configuration object.
       *
       * @since 9.0.0
       */
     ...
     *
     * @param [params]
     * @param [params.dateOnly] If true, only sets FloatDate, not y-axis label
     * @since
     * - 09-2016-19 Only year and month will be displayed in monthly periodicity.
     * - 9.4.0 added parameter `dateOnly`.
     */
    public headsUpHR(params?: {dateOnly?: boolean}): void
    /**
     ...
     * @param  [params.noDrag] If true, hides the drag instructions/button.
     * @param  [params.type] Set to "study","drawing","series", or whatever causes the sticky to be displayed.
     * @param  [params.noText] If true, hides the edit text button.
     ...
     * @return The baseline object of the y-axis baseline renderer if the y-axis has a
     ...
     * @param	[yAxis]	The yAxis to use. Defaults to panel.yAxis.
     * @param [forceStep] Set to true to force step chart; i.e., return the value of the previous bar found.
     ...
     * `""`) counts as a drawing tool and disables crosshairs if the user has not independently enabled them. Set
     ...
     * @since 9.2.0
     */
    public centerCrosshairs(): void
    /**
     * Determines whether crosshairs should be displayed.
     *
     * If user is on mobile or devs have set the flag CIQ.ChartEngine.preferences#displayCrosshairsWithDrawingTool to true,
     * then crosshairs always display when user selects a drawing tool (i.e. when there is an
     * active vectorType) and undisplay when user selects "no tool" (i.e. vectorType = "").
     *
     * Called by CIQ.ChartEngine#doDisplayCrosshair.
     *
     * @return Whether crosshairs should be displayed.
     *
     * @since 9.0.0
     */
    public shouldDisplayCrosshair(): boolean
    /**
     * If crosshairs do not display when a drawing tool is activated, then we change the
     * cursor to a crosshair cursor to show that the tool is active.
     *
     * Called by CIQ.ChartEngine#unDisplayCrosshair.
     *
     * @since 9.0.0
     */
    public shouldDisplayCrosshairCursor(): void
    /**
     ...
     *
     * @param 	symbol	A symbol string, equation or object representing the primary instrument for the chart. **This is a mandatory field and must contain at least one character for the chart to display data, even if not using a primary instrument.**
     ...
     * 													The symbol object also accepts an optional 'display' field, allowing you to assign a custom name to the symbol.
     ...
     * stxx.loadChart({symbol: 'AAPL', display: 'Apple Inc.'}, {
     * 	span: {
     * 		base: 'day',
     * 		multiplier: 2
     * 	},
    ...
    /**
     * Returns the appropriate transformed value for the quote based on the yAxis. If no transformation is found, null is returned.
     * This would be any numeric value
     * @param quote The quote to transform
     * @param yAxis The y-axis for which the transformed data is needed
     *
     * @return A transformed quote value. If no transformation is found, null is returned.
     *
     * @since 9.6.0
     */
    public getTransformedQuote(quote: object, yAxis: CIQ.ChartEngine.YAxis): object
    /**
     * INJECTABLE
     * Animation Loop
     *
     ...
     * @return The visible bars of the dataSegment
     * @since 5.2.0
     */
     ...
     * and the parameter setMarket is set to true,
     ...
     * @param [params.setMarket] If true, then function will use the marketFactory to
     * 		also set the market.
     * @param [params.cb] If set, will be run at end of function.
     *
     * @since
     ...
     * - 9.4.0 Added `params.setMarket`, `params.cb`.
     */
     ...
     * @param [params.exclude-nostore] Set to true to not include series whose noStorage property is set to true.
     *
     ...
     * The CIQ.ChartEngine#displayZone is then created and used to translate dates based on either the local browser's time zone,
     ...
     *
     * - Follow proper LAST SALE format as outlined in [LastSale]CIQ.ChartEngine~LastSale type.
     ...
     * ```
     * <hr>
     * **[LAST SALE format]CIQ.ChartEngine~LastSale**
     ...
     * This data is also used to feed the Depth of Market indicator, [Trade History]WebComponents.TradeHistory and
     * [Order Book]WebComponents.Orderbook web components, part of the [Active Trader package](https://active-trader.demo.chartiq.com/).
     *
     ...
     *   - `destroy`: [destroyEventListener]CIQ.ChartEngine~destroyEventListener
     ...
     *   - `drawingComplete`: [drawingCompleteEventListener]CIQ.ChartEngine~drawingCompleteEventListener
    ...
    /**
     * Sets either an interval or a ResizeObserver to check for chart resizing. See CIQ.resizeDetectInterval
     * for details on which one will be chosen. If the `ms` param is specified here, that value will be used as the
     * `interval` param for CIQ.resizeDetectInterval.
     *
     ...
     *
     * This method is run on chart initialization. To turn off resize, set CIQ.resizeDetectInterval to 0 before
     * initialization.
     *
     * @param [ms] If positive, will set a number of milliseconds for the resize interval. If zero, will turn
     ...
     * 		CIQ.resizeDetectInterval to use an interval instead of a ResizeObserver.
     */
     ...
     *
     * @return The fields — in object-chain form — of the currently rendered objects.
     * @since 7.2.0
     */
     ...
     * @param chart	 The chart object to transform
     * @param transformFunction	 A transformation callback function which takes a number and returns the transformation of that number
     * @param untransformFunction An untransformation callback function
     */
    public setTransform(
    ...
    /**
     * Returns the transformation function for the chart or axis. The chart's transform function is returned if the axis
     * parameter is not provided or if the axis is the chart's yAxis.
     *
     * @param  [yAxis] The axis for which the transform function is needed
     * @param  [returnUntransform] If true, the untransform function is returned; otherwise, the transform function is returned
     *
     * @return The transform function
     *
     * @since 9.6.0
     */
    public getTransformFunction(yAxis?: CIQ.ChartEngine.YAxis, returnUntransform?: boolean): Function
    /**
     * Updates the transform function for the chart or axis. If the axis has a transform function,
     * it is updated; otherwise, the chart's transform function is updated.
     *
     * @param  transformFunction The new transform function
     * @param  [yAxis] The axis for which the transform function is needed
     * @param  [setUntransform] If true, the untransform function is updated; otherwise, the transform function is updated
     *
     * @since 9.6.0
     */
    public updateTransformFunction(
      transformFunction: Function,
      yAxis?: CIQ.ChartEngine.YAxis,
      setUntransform?: boolean
    ): void
    /**
     * INJECTABLE
     * Animation Loop
     ...
     *
     * If you are adding an element containing the word to the DOM and wish the translations services to automatically change
     * to other languages as they are set, use CIQ.ChartEngine#translateUI and pass in the element.
     *
     ...
     * - convert dates to a JS Date in the time zone set by [setTimeZone(dataZone)]CIQ.ChartEngine#setTimeZone.
     ...
     * @return matching panel or undefined if none exists
     * @since 9.1.0
     */
    public getPanelByField(field: string): CIQ.ChartEngine.Panel
    /**
     ...
     * 		deleting it.
     * @param [exclude] Disregard any plot in this array when checking for plots in
     * 		this panel.
    ...
    /**
     * Determines whether a panel has the x-axis attached to it.
     * If CIQ.ChartEngine#xAxisAsFooter is set, the panel with the x-axis is the bottom-most one.
     * Otherwise, it is the main chart panel.
     *
     * @param panel The panel to test.
     * @return true, if the panel contains the x-axis.
     *
     * @since 8.9.0
     */
    public isPanelWithXAxis(panel: CIQ.ChartEngine.Panel): boolean
    /**
     * INJECTABLE
     *
     ...
     * INJECTABLE
     *
     * Adds a panel with a pre-specified percentage. This should be called iteratively when rebuilding a set
     ...
     * @param [params.getDifferentData] If true forces data reload, used for example when extended hours changed to on for the same periodicity
     ...
     * @since 8.9.1 Added `params.getDifferentData`.
     */
     ...
     * CIQ.ChartEngine#setPeriodicity.
     *
     * @return An object literal containing the properties
     ...
     *
     * @return true when the scroll position shows the last tick of the dataSet
     * @since 2016-06-21
     */
     ...
     * @param 		[outZone] A valid time zone from the timeZoneData.js library. This should represent the time zone for the returned date. Defaults CIQ.ChartEngine#dataZone. See CIQ.Market#newIterator for details.
    ...
    /**
     * Sets a default title for the main canvas, for screen readers.
     * Currently default content is available only in English, but can be
     * customized by adding "getCanvasTitle" method to the default configuration
     * object, if used.
     *
     * @since 9.4.0
     */
    public setCanvasTitle(): void
    /**
     ...
     * @param [params.noSlopes] If set, then the chart will draw horizontal bars with no vertical lines.
     * @param [params.step] If set, then the chart will resemble a step chart.  Horizontal lines will begin at the center of the bar.
     ...
     * @param [params.tickFilter] A subset of ticks to plot.
     * @param [params.stretchGaps] Set to true to cause gaps to be represented by a horizontal line.
     ...
     * @param  [pseudo] Pseudo-element to examine (e.g., ":before", or ":after").
     * @return			  An object containing each style, in camel case.
     * @since 8.8.0 Added `pseudo` parameter.
     */
    public canvasStyle(className: string, pseudo?: string): object
    /**
    ...
    /**
     *
     * Converts a string that contains emoji short names to a string with corresponding UNICODE emojis.
     * Valid short names are defined in the stylesheet using class `.emoji_xxx` where `xxx` is the short name,
     * e.g. `.emoji_point_up`.
     * In order for a short name to be converted it must be surrounded by colons (e.g., :point_up:).
     *
     * @param text text to convert, any short names enclosed in colons will become an emoji candidate.
     * @return Input after emoji short names have been converted to UNICODE characters
     *
     * @example
    ...
    /**
     * Enable x-axis titles.
     * The x-axis title, when enabled, can be specified in the parameters, or it can default to CIQ.ChartEngine.XAxis#title.
     *
     * @param params Parameter object.
     * @param [params.enabled] New value for x-axis title display.
     * @param [params.title] If set to a string will override default axis title, setting to null restores default.
     * @since 9.2.0
     * @example
     ...
     */
    public setXAxisTitle(params: {enabled?: boolean, title?: string|null}): void
    /**
     ...
     *		`chart`.[yaxisMarginMultiplier]CIQ.ChartEngine.Chart#yaxisMarginMultiplier.
     */
     ...
     */
    public resetDynamicYAxis(params?: {noRecalculate?: boolean}): void
    /**
     ...
     * @param  [yAxis] Y axis to draw
     * @since 9.6.0 Added an optional YAxis parameter
     */
    public plotYAxisGrid(panel: CIQ.ChartEngine.Panel, yAxis?: CIQ.ChartEngine.YAxis): void
    /**
     * INJECTABLE
     * Animation Loop
     *
     ...
     *
     * It will only draw a label if CIQ.ChartEngine.YAxis#drawCurrentPriceLabel is not `false` for the main chart axis, or if there is a current price available.
     * If you have not loaded enough datapoints to overlap into the current time, as determined by the device's clock, the label will not display.
     *
     ...
     *
     * It will only draw a line if CIQ.ChartEngine.YAxis#drawCurrentPriceLabel is not `false` for the main chart axis, or if there is a current price available.
     ...
     * @since 8.6.0
     */
    public drawCurrentPriceLine(): void
    /**
     ...
     * @param yAxis The y-axis to add (create with new CIQ.ChartEngine.YAxis)
     * @return The y-axis added (or the existing y-axis if a match was found)
     * @since
    ...
    /**
     * Enable y-axis titles.  The y-axis title, when enabled, can be specified in the parameters, or it can default to CIQ.ChartEngine.YAxis#title.
     * If the title is not set anywhere, it will use the y-axis name as the title.
     *
     ...
     *
     * @param [params] Parameters object.
     * @param [params.enabled] New value for x-axis title display on the provided y-axis or enable all y-axis titles at the chart level.
     * @param [params.yAxis] A specific y-axis to alter.  Omit to specify all y-axes.
     * @param [params.title] If set to a string will override default axis title, set to a null will restore default.
     * If yAxis parameter is not provided title is applied to all y-axes on the chart engine.
     * @since 9.2.0
     * @example
     ...
     * @param [params.tickFilter] A subset of ticks to plot.
     * @param [params.isDojiCandleOnly] A flag to preserve the wick color for a paintbar doji.
     * @param [params.volume] Set to true for volume candles.
     * @param [params.stretchGaps] Set to true to cause gaps to be represented by a horizontal line.
     * @param [params.pixelWidth] Amount of width that the candle drawing takes, slightly less than candleWidth. Defaults to 65% of candleWidth.
     ...
     * @param [parameters.tickFilter] A subset of ticks to plot.
     * @param [parameters.stretchGaps] Set to true to cause gaps to be represented by a horizontal line.
     ...
     * CIQ.ChartEngine.YAxis#minimumPriceTick can be set to specify that the y-axis vertical grid be drawn with specific ranges; for example, <code>stxx.chart.panel.yAxis.minimumPriceTick=.25</code>
     *
     ...
     * @param [parameters.yAxis] The y-axis to create. Defaults to panel.yAxis.
     */
    public createYAxis(
      panel: CIQ.ChartEngine.Panel,
      parameters?: {
    ...
    /**
     * Draws the Yaxis clamped to min/max values when set with the yaxis `setClamp` method.
     * This method is called automatically by CIQ.ChartEngine#drawYAxis when the `clamp` property is set.
     *
     * @param  panel The panel to draw the y-axis
     * @param  parameters Parameters for the y-axis. See CIQ.ChartEngine#drawYAxis
     * @since 9.5.0
     */
    public drawYAxisFixed(panel: CIQ.ChartEngine.Panel, parameters: object): void
    /**
  ...
  class Plotter {
     ...
     * @param [meta] Object containing metadata
     * @param [meta.centered] Should draw text centered
     * @param [meta.vertical] Should draw text vertically; pass `CIQ.Plotter.BOTTOM_TO_TOP` or `CIQ.Plotter.TOP_TO_BOTTOM`
     * @since 9.2.0 added metadata param to support vertical text
     */
    public addText(
      name: string,
      text: string,
  ...
  class Renderer {
     ...
     * @param  [parameters.labels] Draw labels when set for histogram and line renderers
     * @param  [parameters.labels.backgroundColor] Label background color override
     * @param  [parameters.labels.textColor] Label text color override
     * @param  [parameters.labels.font="12px Arial"] Label font override
     * @param  [parameters.labels.bold] Whether the font should be bold
     * @param  [parameters.labels.italic] Whether the font should be italic
     * @param  [parameters.labels.padding] Label padding override
     * @param  [parameters.labels.verticalOffset] Label vertical offset override
     * @param  [parameters.labels.insideBar] Whether to draw labels inside histogram bars
     * @param  [parameters.labels.lastVisibleOnly] Draw only the last visible label
     * @return            Returns a copy of this for chaining
     * @since 5.1.0 Added `fillStyle`, `baseColor`, and `bgColor` parameters.
     * - 9.6.0 Added `parameters.priority` and `parameters.labels` object with
     * 		backgroundColor, textColor, font, bold, italic, padding, verticalOffset, insideBar and lastVisibleOnly properties
     * @example
     *	// add multiple series and attach to a custom y-axis on the left.
     *
     *	// note how the addSeries callback is used to ensure the data is present before the series is displayed
     ...
     * @param  [preserveSeries=false] Set to `true` to keep the series data in the CIQ.ChartEngine objects, otherwise it will be deleted.
   ...
   * @param [extendUndefinedProp] If true then undefined properties will copy to target object
   ...
   * - 9.4.0 added `extendUndefinedProp` parameter.
   */
  function extend(
    target: object,
    source: object,
    shallow?: boolean,
    extendUndefinedProp?: boolean
  ): object
   ...
   * @param parameters.noSlopes If set, then the chart will fill rectangles with no transition lines between levels
   ...
   * @param  [borderColor] The color for the border of the item
   * @return       A tuple containing the X,Y position for the next the item
   * @since 9.1.0 Added `borderColor` parameter.
   */
   ...
   * interchange of hex, RGB, and RGBA colors.
   * @param  color A CSS color or array of colors.
   * @return       The native formatted color or array of colors.
   * @since 8.8.0 Allow use of array as parameter and return value.
   */
  function convertToNativeColor(color: string|string[]): string|string[]
   ...
   * This function will pierce shadow roots when traversing.
   *
   ...
   * @param  useTimeout Whether to apply a timeout or not. If number > 0, then the number of milliseconds.
   */
   ...
   * @param [win] Optional top level window, if not the main window where library is loaded
   * @return Height of page in pixels
   * @since 8.5.0 Added `win` parameter.
   */
  function pageHeight(win?: Window): number
   ...
   * @since 8.5.0 Added `win` parameter.
   */
  function pageWidth(win?: Window): number
   ...
   *
   * @param obj The DOM element or an object in the element to update.
   ...
   */
  function efficientDOMUpdate(obj: object, member: string, value: string): boolean
   ...
   * @param [params.preventUnderlayClick=true] By default, prevents an underlying element from being "clicked" on a touch device 400ms after the overlay was tapped. Set to false for input fields or any div containing input fields (body).
   * @param [params.absorbDownEvent=true] Ensures that a mousedown, pointerdown, or touchstart event doesn't get passed to the parent.
   * @param [params.preventClickEvent=false] Prevents "click" event from firing. Potentially useful when bubbling events emanated from screen readers.
   * @param [params.keyboardClick=false] Sets a `keyup` listener to dispatch a click event when "Space" or "Enter" are pressed.
   * @since
   ...
   * @param [doc] document to interact with
   * @since
   * - 6.3.0
   * - 8.5.0 Added optional `doc` parameter.
   */
  function smartHover(doc?: Document): void

  /**
   * Converts a node to one which will translate its inner text when another language is chosen.
   * If there is no translationCallback, the node will not be translatable.
   *
   * @param  element The element to convert
   * @param  stx The chart object
   * @param  original The untranslated text
   * @param [language] Language. Defaults to CIQ.I18N.language.
   *
   * @since 8.8.0
   */
  function makeTranslatableElement(
    element: HTMLElement,
    stx: CIQ.ChartEngine,
    original: string,
    language?: string
  ...
  /**
   * Adds translatable aria-label attribute.
   * If there is no translationCallback, the node will not be translatable.
   *
   * @param  element The element to convert
   * @param  stx The chart object
   ...
   * @since 9.1.0
   */
  function addTranslatableAriaLabel(
    element: HTMLElement,
    stx: CIQ.ChartEngine,
    original: string,
    language?: string
   ...
   * @param bustShadowRoots Set to true to penetrate shadow root boundaries.
   ...
   *
   * **Note:** If an interval is passed to CIQ.resizeObserver, that value will be used instead.
   * See CIQ.resizeObserver for more details.
   *
   * @since 8.6.0
   */
  let resizeDetectInterval: number
   ...
   * @param [resizeHandle] A handle to the resizer, which is null the first time the function is
   * 		called, or is the return value of the function for subsequent calls. Can be a ResizeObserver.
   * @param [interval] Optionally specify an interval to use for checking instead of a ResizeObserver.
   * 		Set this value to 0 to turn off the observer. Defaults to 0 if listener is null and resizeHandle is available.
   ...
   * 		the CIQ.resizeDetectInterval is positive.
   */
  function resizeObserver(
    element: HTMLElement,
    listener: Function|null,
    resizeHandle?: object|number|null,
    interval?: number
  ): object
   ...
   */
  let localStorage: Record<string, Function>
   ...
   * @return object containing internally compliant periodicity, interval, and timeUnit
   * @since 5.1.1
   */
  function cleanPeriodicity(period: number, interval: number, timeUnit: string): object
  function cleanPeriodicity(period: number, timeUnit: string): object
   ...
   * Rounds off decimal values to 4 places before condensing.
   *
   ...
   * @since 9.2.0 Rounds off decimal values to 2 or 4 places before condensing.
   */
   ...
   * @param origin Initial x and y coodinates ( example  {x:50, y:25} ).
   * @param point	Final x and y coodinates ( example  {x:50, y:25} ).
   * @param radius The radius value to check, indicating linear distance.
   * @return  true if within the radius.
   * @since 8.9.0
   */
  function withinRadius(origin: object, point: object, radius: number): boolean

  /**
   * Checks a semantic version to determine if it is compatible with another.
   * Patch differences are considered compatible.
   * Minor differences are considered incompatible unless allowMinor flag is set.
   * Major differences are considered incompatible.
   * @param testVersion Semantic version to compare.
   * @param compareVersion Semantic version to compare against.
   * @param allowMinor True to consider a minor discrepancy to be compatible.
   ...
   * @param  [withoutPrototypeProperties] If true do not copy prototype properties.
   * @return      A deep clone of the "from" object
   * @since 9.2.0 added withoutPrototypeProperties parameter
   */
  function clone(
    from: object,
    to?: object,
    withoutPrototypeProperties?: boolean
  ): object
   ...
   * @param  a First object
   * @param  b Second object
   * @return True if object a contains object b
   * @since 9.1.0
   */
  function isSubsetOf(a: object, b: object): boolean
  ...
  /**
   * Decorates a method by pre or post executing the provided side effect function `fn`. This decorator should be used only if a
   * chart engine event listener or injection is not working or not available.
   *
   * @param obj Object holding the method to decorate
   * @param methodName Method name to modify
   * @param fn Function to apply sideffects when the original function is run. Function cannot be an arrow function.
   * @param [postExecution=true] If true execute supplemented function after original, if false execute it before original.
   * 		If the method to modify is async and postExecution is set to true this wrapper is not waiting for the method returned Promise to be resolved
   * 		prior applying provided sideffect `fn` function
   * @return Restore function to restore original definition
   * @since 9.4.0
   */
  function decorateFn(
    obj: object,
   ...
   * @param [doc] document to attach script to.
   *
   * @since
   ...
   * @param [doc] document to attach stylesheet to.
   * @since
   * - 2016-03-11
   * - 8.5.0 Added the `doc` parameter.
   */
  function loadStylesheet(stylesheet: string, cb: Function, doc?: Document): void
   ...
   * @param [doc] document plugins are loaded in.
   * @since
   * - 6.1.0
   * - 8.5.0 Added the `doc` parameter.
   */
  function waitForPlugins(plugins: any[], cb: Function, doc?: Document): void
   ...
   * - 8.5.0 Added the `doc` parameter.
   */
  function addInternalStylesheet(content: string, path: string, doc?: Document): void
   ...
   *
   * Draws a rectangle on the canvas, with an arrowhead on the screen, using CIQ.roundRect with an `edge` setting of "arrow".
   ...
   * Displays a floating label over the y-axis.
   *
   * Draws a rectangle on the canvas, with just the right side curved corners, using CIQ.roundRect with an `edge` setting of "flush".
   * It then calls CIQ.createLabel to print the text over this background shape; which can be customized to control the text format for these labels.
   *
   * Visual Reference:
   ...
   * Displays a floating label over the y-axis.
   *
   * Draws a rectangle on the canvas using CIQ.roundRect with a `radius` of 0
   * It then calls CIQ.createLabel to print the text over this background shape; which can be customized to control the text format for these labels.
   *
   * Visual Reference:
   * ![rect](rect.png "rect")
   ...
   * Displays a floating label over the y-axis.
   *
   * Draws a 'ticked' rectangle on the canvas, using CIQ.roundRect.
   * It then calls CIQ.createLabel to print the text over this background shape; which can be customized to control the text format for these labels.
   *
   * Visual Reference:
   ...
   * @since 8.8.0
   */
  function getActiveElement(root?: HTMLElement): HTMLElement
   ...
   *
   * @param enable A boolean that sets the current state of the metrics server client
   * @return config The metrics client config object
   * @return config.clientId The client's uuid
   * @return config.url The URL of the server to which all logging requests are sent
   * @since 8.8.0
   */
  function metricsClient(enable: boolean): object
}
...
export namespace CIQ.Renderer {
  ...
  class Lines {
     ...
     * @param [config.params.centered] To be paired with the baseline. Set to true to center the chart around the baseline. If the baseline isn't set, this setting is ignored.
 ...
 * Timezone Library
 *
 * This will be an empty object unless standard.js is also imported (or at least js/standard/timezone.js).
 *
 */
export namespace timezoneJS {
  /**
   * If true, indicates this is stub function and requires import of standard.js to use.
   *
   */
  let stub: boolean
}
...
export namespace CIQ.Renderer.Lines {
   ...
   * 	line, mountain, baseline (delta), baseline (noncentered), step, vertex, colored, wave
