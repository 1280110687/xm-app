...
class DialogContentTag extends BaseComponent {
   ...
   * @return {HTMLElement[]} Array containing all tab navigable elements.
   *
   * @tsmember CIQ.UI.DialogContentTag
   * @since 8.8.0
   */
...
CIQ.UI.HeadsUp.prototype.begin = function () {
  ...
  if (this.context.stx.translateUI) this.context.stx.translateUI(this.node);
  var params;
  if (this.params.followMouse) {
    ...
    CIQ.extend(params, this.params); // Override default marker setup by passing marker parameters into CIQ.UI.HeadsUp
  ...
  if (drawing) {
    this.clickListener = (e) => {
      const onSettings =
        ...
        e.target.closest("cq-dialog-overlay") ||
        e.target.closest("cq-color-picker");
      const isModal = stx.uiContext.isModal();
      if (!onSettings && !isModal) this.endEdit({}, "close"); //  Exit drawing setting edit mode if clicked outside settings
    };
...
DrawingEdit.prototype.edit = function () {
  if (this.drawingContext) {
    ...
    const localContext = this.context.topNode;
    // The drawing context menu element may be created outside of any instant chart so also query the document
    const contextContainer = (
      localContext.querySelector("cq-drawing-context") ||
      localContext.ownerDocument.querySelector("cq-drawing-context")
    ).parentNode;
    let settingsPalette =
      localContext.querySelector("cq-drawing-settings") ||
      localContext.ownerDocument.querySelector("cq-drawing-settings");
    ...
    if (settingsPalette && !/break.*sm\b/.test(container.className)) {
      ...
      let { left, top } = contextLocation;
      if (drawing.name === "annotation" || drawing.name === "callout") {
        top += settingsPalette.clientHeight / 3;
      }
      settingsPalette.detach(left, top);
    }
    ...
    if (["static", "dynamic"].includes(headsUpType)) {
      if (!this[headsUpType]) {
        ...
        this.context.topNode
          ...
          .forEach((area) => {
            if (!area.querySelector("cq-hu-" + headsUpType)) {
...
CIQ.UI.Layout.prototype.setHeadsUp = function ({ node }, headsUpType) {
  const multiChartContextEl =
    this.context.topNode.parentElement.closest("cq-context");
  const charts = multiChartContextEl
    ? multiChartContextEl.getCharts()
    : [this.context.stx];
  charts.forEach((stx) => {
    const { layout } = stx;
    let { headsUp } = layout;
 ...
 * @param {string} timeUnit Same as `timeUnit` from {@link CIQ.ChartEngine#setPeriodicity}.
 *
 * @memberof CIQ.UI.Layout
 */
CIQ.UI.Layout.prototype.setPeriodicity = function (
  ...
  var self = this;
  if (self.context.loader) self.context.loader.show(true);
...
CIQ.UI.Layout.prototype.chartType = function (node) {
  ...
  const listener = ({ obj: { chartType, aggregationType } }) => {
    ...
    let useLabel = type;
    if (iconNode) {
      iconNode.className = `icon generic ${iconNode.classList.contains("hidden") ? "hidden " : ""
 ...
 *
 * @memberof CIQ.UI.Keystroke
 * @param {string} key Key Code of command key
 * @return {string} Internal representation of key
 */
...
CIQ.UI.KeystrokeHub.executeHotkeyCommand = function ({
  ...
  switch (action) {
    ...
    case "toggleDrawings":
      ...
      else stx.chart.hideDrawings = !stx.chart.hideDrawings;
      break;
    default:
...
class UIManager extends HTMLElement {
  ...
  ifAllClosed() {
    if (!this.activeMenuStack.length) {
      Array.from(
        (this.document || document).querySelectorAll(
          "cq-context,*[cq-context]",
        ),
 ...
 * @return {CIQ.UI.UIManager} A reference to the document's UI manager.
 *
 * @memberof CIQ.UI
 * @since
 * - 7.5.0
...
class Chart {
  ...
  createChartAndUI({ container, config }) {
    ...
    let uiContext = new CIQ.UI.Context(stx, contextContainer); // 触发初始化Loading
   ...
   * @return {Function} A callback that sets the active context of the keystroke hub.
   *
  ...
  initDialogHandler(uiContext) {
    ...
    function handleDialog({ type = null, params = {} } = {}) {
      ...
      let el = qs(itemConfig.tag, uiContext.topNode) || qs(itemConfig.tag, doc);
      if (!el) {
  ...
  loadChart(uiContext) {
    ...
    function loadSymbol() {
      if (stx.crossSection) {
        let chartType = stx.layout.chartType.includes("crosssection")
          ? stx.layout.chartType
          : stx.crossSection.initialSettings.chartType || "crosssection";
   ...
   *
   * @param {CIQ.UI.Context} uiContext The chart user interface context.
   * @return {Function} A function that sets the size of the side navigation panel.
   *
   ...
   *
   * @param {CIQ.UI.Context} uiContext The chart user interface context.
   * @return {Function} A function that sets the top position of the chart area.
   *
   ...
   *
   * @param {CIQ.UI.Context} uiContext The chart user interface context.
   * @return {Function} A function that sets the width of the side panel.
   *
   *
   ...
   *
   * @param {CIQ.UI.Context} uiContext The chart user interface context.
   * @return {Function} A function that sets the responsive break point of the UI context.
   *
   ...
   *
   * @param {CIQ.UI.Context} uiContext The chart user interface context.
   * @return {Function} A function that sets the class of the UI context.
   *
   * @alias classSetter
   * @memberof CIQ.UI.Chart.prototype
   * @since 8.8.0
...
class Multichart {
  ...
  createCharts({ containerId, chartsConfig }, config) {
    ...
    config.hotkeyConfig.hotkeys = config.hotkeyConfig.hotkeys.filter(
      ({ action, label }) =>
        !/\btab\b/i.test(label) && action !== "toggleDrawings",
    );
  ...
  addChart(container, config, chartTemplate) {
    container.addChart = (options, position = "after") => {
      ...
      // backwards compat with legacy webcomponents
      container
        .querySelectorAll("cq-info-toggle-dropdown")
        .forEach((item) => item.initInfoComponents(container));
      const areaContainer = stx.container.closest(".ciq-chart-area");
      ["static", "dynamic"].forEach((mode) => {
        const [masterChart] = container.getCharts().slice(-1);
        if (
          masterChart.uiContext.getAdvertised("Layout")[mode] &&
          !areaContainer.querySelector("cq-hu-" + mode)
        )
          areaContainer.appendChild(document.createElement("cq-hu-" + mode));
      });
   ...
   *
   * @param {CIQ.ChartEngine} stx A reference to the chart engine.
   * @return {function} A function that can be called to unfreeze the engine.
   *
 ...
 * 		{@link Faquery}.
 * @return {object} A jQuery object if jQuery is enabled and `force` is false; otherwise,
 ...
 * You will need to define `render` and `unrender` functions on your class to place your markup (and remove it).
 ...
 * @param {CIQ.ChartEngine} params.stx A reference to the chart to which the plug-in is added.
 * @param {CIQ.UI.Context} params.context A reference to the user interface context.
 * @param {HTMLElement|string} params.container The DOM element to which the plugin toggle is attached.
 * @param {string} params.markup A markup string to use for the button that toggles the plug-in.
 * @param {string} [params.location="top"] Where the plugin appears, "top" and "right" supported.
 *
 * @alias CIQ.UI.SidePanelPlugin
 * @since 9.5.0
 * @class
 */
class SidePanelPlugin {
  constructor(params, pluginName) {
  ...
  addToDom() {
    const { params, pluginName } = this;
    const {
      topNode,
      topNode: { multiChartContainer },
    } = params.context;
    const container = multiChartContainer || topNode;
    if (!container.querySelector(`div.${pluginName}-container`)) {
      const node = document.createElement("div");
      node.classList.add("ciq-" + pluginName, params.location);
      node.setAttribute("disabled", "");
      let pluginContainer =
        params.location === "right"
          ? container.querySelector("cq-side-panel")
          : container.querySelector(".cq-plugin-container");
    ...
    if (
      params.container &&
      !params.container.querySelector(`[member=${pluginName}]`)
    ) {
      const div = document.createElement("div");
      CIQ.innerHTML(div, params.toggleMarkup);
      [...div.childNodes].map((node) =>
  ...
  subscribe() {
    ...
    if (config && !config.channels[pluginName])
      config.channels[pluginName] = `channel.${pluginName}`;
    const channel =
      (config.channels || {})[pluginName] || `channel.${pluginName}`;
    this.activationSubscription = BaseComponent.prototype.channelSubscribe(
      channel,
      (isActive) => {
        if (this.node) {
          if (isActive) {
            ...
            if (this.unrender) this.unrender();
          }
        }
      },
      params.context.stx,
...
CIQ.UI.SidePanelPlugin = SidePanelPlugin;
export { CIQ };
