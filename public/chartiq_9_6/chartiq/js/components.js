...
let __js_webcomponents_dropdown_ = (_exports) => {
  ...
  Dropdown.itemTemplate = ({
    ...
    const classString = `${className ? `${className} ` : ""}${active ? "ciq-active " : ""
      }`;
    const params =
      value && value !== 0
        ? Array.isArray(value)
  ...
  ShowRange.itemTemplate = ({
    ...
    return `
		...
		>\
			${label
        ? type === "heading"
          ? `<h4><cq-heading class="inline" text="${label}"${id ? ` id="${id}"` : ""
          }></cq-heading></h4>`
          : `
...
let __js_webcomponents_yAxisContext_ = (_exports) => {
  ...
  function renderTransformItem(param, label, active) {
    return `
		<div stxtap="updateYAxis('transform', '${param}')" keyboard-selectable="true" class="ciq-switch-item ciq-transform ciq-${param} ${active ? "ciq-active" : ""
