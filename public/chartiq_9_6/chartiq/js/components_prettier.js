      ...
      displayAttribution(stx) {
        ...
        if (chart.attribution) {
          ...
          if (source + exchange != chartAttrib.getAttribute("last-attrib")) {
            ...
            chartAttrib.querySelector("[attrib-quote-type]").innerHTML =
              exchange;
  ...
  class ChartInstructions extends CIQ.UI.ContextTag {
    ...
    setContext(context) {
      ...
      if (hotkeyConfig.hotkeys) {
        const keyConfigs = hotkeyConfig.hotkeys.filter(
          (conf) => conf.ariaLabel,
        );
...
let __js_webcomponents_chartTitle_ = (_exports) => {
   ...
   */
  ChartTitle.markup = `
		<cq-clickable role="button" title="Symbol Search">
        ...
        for (let s in stx.chart.series) {
          ...
          if (price && prices && prices.data) {
            ...
            if (pdSymbol !== null) {
              ...
              if (price.innerText !== paddedPrice)
                price.innerText = paddedPrice;
            }
          }
        }
      };
    ...
    renderLegend() {
      ...
      for (let r in stx.chart.seriesRenderers) {
        ...
        for (let s = 0; s < renderer.seriesParams.length; s++) {
          ...
          let color = seriesParameters.color || rSeries.color;
          ...
          if (q || q === 0) {
            ...
            if (price) {
              if (renderer.params.yAxis) {
                price.innerText = stx.formatYAxisPrice(
                  q,
                  stx.panels[renderer.params.panel],
                  renderer.params.yAxis.decimalPlaces,
                  renderer.params.yAxis,
                );
              } else {
          ...
          if (
            btn &&
            ((!seriesParameters.color && !rSeries.color) ||
              seriesParameters.permanent)
          )
...
let __js_webcomponents_cvpController_ = (_exports) => {
  ...
  class CVPController extends CIQ.UI.ContextTag {
    ...
    setStyle(activator, width = "1", pattern = "dotted") {
      ...
      this.context.stx.currentVectorParameters["pattern" + this._scope] =
        pattern;
  ...
  class Dialog extends CIQ.UI.BaseComponent {
    ...
    hide() {
      ...
      if (
        ...
        while (caller && !CIQ.trulyVisible(caller))
...
let __js_webcomponents_doubleSlider_ = (_exports) => {
   ...
   * @alias WebComponents.DoubleSlider
   * @class
   * @extends CIQ.UI.BaseComponent
   * @since 8.3.0
   *
  ...
  class DoubleSlider extends CIQ.UI.BaseComponent {
     ...
     *
     * @alias init
     * @tsmember WebComponents.DoubleSlider
    ...
    init() {
      ...
      ["min", "max", "step", "low", "high"].forEach((prop) => {
        if (this.hasAttribute(prop))
          this[prop] = Number(this.getAttribute(prop));
      });
...
let __js_webcomponents_dropdown_ = (_exports) => {
  ...
  class Dropdown extends CIQ.UI.BaseComponent {
    ...
    populate() {
      ...
      if (Array.isArray(contentArr)) {
        while (this.contentRoot.firstChild)
          this.contentRoot.firstChild.remove();
    ...
    resize() {
      ...
      const context = CIQ.UI.closestContextContainer(contentRoot);
      if (context) {
...
let __js_webcomponents_fibSettingsDialog_ = (_exports) => {
  ...
  class FibSettingsDialog extends CIQ.UI.DialogContentTag {
     ...
     */
    add() {
      let level = this.querySelector(
        "[cq-custom-fibonacci-setting] input",
      ).value;
      if (!level) return;
...
let __js_webcomponents_floatingWindow_ = (_exports) => {
  ...
  class DocWindow {
    ...
    bindEvents() {
      ...
      w.ownerDocument.addEventListener(
        "mousemove",
...
let __js_webcomponents_instantChart_ = (_exports) => {
   ...
   * @alias WebComponents.InstantChart
   * @class
   * @extends HTMLElement
   * @example
    ...
    class LanguageDialog extends CIQ.UI.DialogContentTag {
      ...
      open(params) {
        ...
        for (let langCode in languages) {
          ...
          node.querySelector("cq-language-name").innerText =
            languages[langCode];
...
let __js_webcomponents_lookup_ = (_exports) => {
  ...
  if (!CIQ.ChartEngine.Driver) {
    ...
    class Lookup extends CIQ.UI.ContextTag {
       ...
       *
       * @param {object[]} arr The array of results.
       *
       * @tsmember WebComponents.Lookup
       */
      results(arr) {
...
let __js_webcomponents_lookupDialog_ = (_exports) => {
  ...
  class LookupDialog extends CIQ.UI.DialogContentTag {
     ...
     * @param {Function} [params.callback] callback called when an item is selected.
     * @param {Boolean} [params.useFirstResult] whether to return the first result when hitting enter.
     *
     * @tsmember WebComponents.LookupDialog
     *
     * @since 9.0.0 respect caller element cq-maintain-case for symbols added from input box.
     * @since 9.5.0 added useFirstResult
     */
    open(params) {
      const { caller, context, callback, useFirstResult } = params;
      ...
      if (callback) {
        ...
        this.selectItem = LookupDialog.prototype.selectItem.bind(this);
      }

      this.useFirstResult = useFirstResult;
    ...
    initialize() {
      ...
      this.form.addEventListener("submit", (e) => {
        e.preventDefault();
        this.selectItem(
          (this.useFirstResult ? this.firstResult : null) || this.input.value,
        );
      });
      ...
      this.filters.addEventListener("mousedown", (e) => {
        this.context.stx.ownerWindow.requestAnimationFrame(() =>
          e.target.blur(),
        );
      });
    ...
    results(resultsArr) {
      ...
      this.firstResult = resultsArr[0].data;
...
let __js_webcomponents_palette_ = (_exports) => {
  ...
  class Palette extends CIQ.UI.ContextTag {
    ...
    checkPosition() {
      ...
      coordinates[0] = Math.min(
        coordinates[0] + this.dragStrip.clientWidth * 0.5,
        parentBounds.width - this.clientWidth,
      );
      coordinates[1] = Math.min(
        coordinates[1] + this.dragStrip.clientHeight * 0.5,
        parentBounds.height - this.clientHeight,
      );

      this.setTransform(coordinates[0], coordinates[1]);
    }
    ...
    setTransformPosition(x, y) {
      ...
      nextTop =
        Math.min(
          ...
          parentBounds.top +
            parentBounds.height -
            (this.dragStrip.clientHeight + this.dragMargin),
        ) - chartBounds.top;
      ...
      nextLeft =
        Math.min(
          ...
          parentBounds.left +
            parentBounds.width -
            this.clientWidth -
            (this.dragStrip.clientWidth + this.dragMargin),
        ) - chartBounds.left;
...
let __js_webcomponents_scroll_ = (_exports) => {
  ...
  class Scroll extends CIQ.UI.BaseComponent {
    ...
    resize() {
      ...
      if (!contextHeight) return;
      let height =
        contextHeight - (position.top - contextTop) - reduceMenuHeight;
      const holders = CIQ.climbUpDomTree(
        this,
        ".stx-holder,.stx-subholder,.chartContainer",
        true,
    ...
    class ShareDialog extends CIQ.UI.DialogContentTag {
      ...
      updateSocialLinks(type, encodedMessage) {
        ...
        if (chartSharing && chartSharing.quickLinks) {
          ...
          [
            ...this.querySelectorAll(
              `.share-${type}-container .ciq-share-link`,
            ),
          ].forEach((button) => {
  ...
  class SideNav extends CIQ.UI.ContextTag {
    ...
    setContext(context) {
      ...
      const setActive = () => {
        ...
        const headsUpValue = this.channelRead(headsUpChannel);
        ...
        if (isBreakSm) {
          const newValue = CIQ.clone(headsUpValue);
          newValue.static = true;
          if (crosshairActive) {
            this.channelWrite(headsUpChannel, newValue);
            // need to toggle off and on or hu markers duplicate
            this.channelWrite(crosshairChannel, false);
            this.channelWrite(crosshairChannel, true);
          }
        }
      };
...
let __js_webcomponents_sidePanel_ = (_exports) => {
  ...
  class SidePanel extends CIQ.UI.ContextTag {
    ...
    setContext(context) {
      ...
      if (!this.mo) {
        const observeAttributes = (child) =>
          this.mo.observe(child, { attributes: true });
        this.mo = new MutationObserver((records) => {
          [...records[0].addedNodes].forEach(observeAttributes);
          if (records[0].attributeName) handleOpen(this.nonAnimatedWidth() > 0);
        });
        this.mo.observe(this, { childList: true });
        [...this.children].forEach(observeAttributes);
      }
      this.channelSubscribe(
        channels.breakpoint || "channel.breakpoint",
        handleSizeChanges,
    ...
    /**
     * Closes the side panel and all of its child plugins which were opened using the {@link WebComponents.SidePanel#open} function.
     *
     * @tsmember WebComponents.SidePanel
     */
    close() {
    ...
    class StudyDialog extends CIQ.UI.DialogContentTag {
      ...
      refreshInputs(empty) {
        ...
        for (let i = 0; i < this.helper.inputs.length; i++) {
          ...
          } else if (input.name === "Comparison Symbol") {
            formField = this.ownerDocument.createElement("cq-symbol-input");
            formField.initialize({ value: input.value });
            this.setChangeEvent(formField, "inputs", input.name);
          } else if (
            input.type == "text" ||
            input.type == "date" ||
          ...
          if (!oldInput) {
            ...
            oldInput.originalOuterHTML !== newInput.originalOuterHTML
          ) {
      ...
      refreshOutputs(empty) {
        ...
        for (let i = 0; i < this.helper.outputs.length; i++) {
          ...
          const id =
            "cq-study-dialog-outputs-" + output.name.replace(/ /g, "_");
      ...
      refreshParameters(params, empty) {
        ...
        for (let i = 0; i < this.helper.parameters.length; i++) {
          ...
          if (!oldParam) {
            ...
            oldParam.originalOuterHTML !== newParam.originalOuterHTML
          ) {
  ...
  if (!CIQ.Studies) {
    ...
    class StudyLegend extends CIQ.UI.ModalTag {
      ...
      renderLegend() {
        ...
        if (
          this.series === "true" &&
          !this.root.querySelector("cq-comparison")
        ) {
      ...
      setContext(context) {
        ...
        if (cloneToPanels) {
          CIQ.UI.closestContextContainer(this).classList.add(
            "stx-panel-legend",
          );
...
let __js_webcomponents_symbolInput_ = (_exports) => {
  /* global _CIQ, _timezoneJS, _SplinePlotter */
   ...
   * _**Emitters**_
   *
   * A custom event will be emitted by the component when clicked.
   * See {@link CIQ.UI.BaseComponent#emitCustomEvent} for details on how to listen for this event.
   * The details of the event contain the following:
   * | property | value |
   * | :------- | :---- |
   ...
   * This component comes with a default markup which is used when the component tag contains no other markup when it is added to the DOM.
   *
   * @alias WebComponents.SymbolInput
   * @extends CIQ.UI.ContextTag
   * @class
   * @protected
   * @since 9.5.0
   */
  class SymbolInput extends CIQ.UI.ContextTag {
    static get observedAttributes() {
      return [];
    }
    ...
    adoptedCallback() {
      super.adoptedCallback();
      CIQ.UI.flattenInheritance(this, SymbolInput);
      this.constructor = SymbolInput;
    }
     ...
     * @param {Object} params Parameters
     * @param {String} [params.value] Input symbol
     *
     * @tsmember WebComponents.SymbolInput
     */
    initialize(params) {
      this.addDefaultMarkup();
      this.currentSymbol = this.querySelector(".current-symbol");
      this.currentSymbol.value = params.value;
      this.value = params.value;
    }

    /**
     * Open the symbol input dialog
     *
     * @tsmember WebComponents.SymbolInput
     */
    openDialog() {
          ...
          channel,
          {
            ...
            params: {
              ...
              callback: (symbolData) => {
                ...
                this.currentSymbol.value = symbolData.symbol;
              },
            },
          },
          context.stx,
  ...
  SymbolInput.markup = `
	...
	`;
  CIQ.UI.addComponentDefinition("cq-symbol-input", SymbolInput);
};
    ...
    class ThemeDialog extends CIQ.UI.DialogContentTag {
      ...
      applyChanges() {
        const multiChartContainer =
          this.context.topNode.parentElement.closest("cq-context");
        const charts =
          multiChartContainer && multiChartContainer.getCharts
            ? multiChartContainer.getCharts()
            : [this.context.stx];
      ...
      open(params) {
        ...
        const configurePiece = (name, obj, field, type) => {
          const piece = this.querySelector(
            `cq-theme-piece[cq-piece="${name}"]`,
          );
          if (!piece) return;
      ...
      save() {
        ...
        this.qsa("cq-themes", null, true)
          .filter((n) => n.ownerDocument === this.ownerDocument)
          .forEach((t) => {
...
let __js_webcomponents_themes_ = (_exports) => {
  ...
  class Themes extends CIQ.UI.ContextTag {
    ...
    /**
     * Create the inner markup of the component.  The UI for the component will resemble a menu of choices,
    ...
    loadBuiltIn(className) {
      if (this.currentLoadedBuiltIn) {
        this.currentLoadedBuiltIn.split(/\./).forEach((name) => {
          this.removeThemeClass(name);
        });
      }
    ...
    loadCustom(themeName) {
      ...
      if (theme.builtIn) {
        this.addThemeClass(theme.builtIn);
      }
    ...
    addThemeClass(name) {
      const addClass = (container) => {
        ...
        container.classList.add(...name.split("."));
  ...
  if (!CIQ.timeZoneMap) {
    ...
    class TimezoneDialog extends CIQ.UI.DialogContentTag {
      ...
      open(params) {
        ...
        const displayZone = () => {
          const currentUserTimeZone = this.querySelector(
            ".currentUserTimeZone",
          );
          if (stx.displayZone) {
        ...
        if (!this.template) {
          this.template = ul
            .querySelector("li.timezoneTemplate")
            .cloneNode(true);
        }
...
let __js_webcomponents_toggle_ = (_exports) => {
  ...
  class Toggle extends CIQ.UI.ContextTag {
    ...
    handlePropertyChange(name, oldValue, newValue) {
      ...
      if (name === "stxtap")
        CIQ.UI.BaseComponent.buildReverseBindings(this.root);
      else if (this.usingMarkup) {
        this.reset();
      } else {
        // do nothing when using predefined content
     ...
     */
    connectObservable() {
      const { objectsMember: member, obj } = this;
      if (member && obj) {
        this.disconnectObservable();
        const listener = () => {
    ...
    set(value) {
      ...
      if (this.member) {
        ...
        if (this.callbacks)
          this.callbacks.forEach((cb) => cb.call(this, value));
        this.value = value;
      }
      this.updateClass();
      this.setAriaPressed();
...
let __js_webcomponents_viewDialog_ = (_exports) => {
  ...
  class ViewDialog extends CIQ.UI.DialogContentTag {
    ...
    save() {
      ...
      (
        this.context.topNode.multiChartContainer || this.context.topNode
      ).CIQ.UI.Components.filter(
    ...
    update(activator, field, value) {
      ...
      if (!isTemplate) {
        ...
        if (field === "decoration") {
          ...
          this[field] = value;
          this[field + "Header"].setAttribute(
            "text",
            u ? u.join(" ") : "- - -",
          );
        }
        if (field === "showLines") {
          ...
          this.lineToggleCheckbox.classList[
            value === "true" ? "add" : "remove"
          ]("ciq-active");
        ...
        if (newTemplate) {
          ...
          this.impulseHeader.setAttribute(
            "text",
            newTemplate.impulse.join(" "),
          );
...
let __js_webcomponents_yAxisContext_ = (_exports) => {
   ...
   *
   * This component appears as a "hamburger menu" when a yaxis is hovered, and as a context menu when the yaxis is right-clicked.
   * A menu of actions are displayed relevant to that yaxis.
   *
   * _**Attributes**_
   *
   * The following attributes are supported (but not observed for changes):
   * | attribute   | description |
   * | :---------- | :---------- |
   * | cq-allow-transform      | Display of the transform menu toggles. Defaults to on, set value to "false" to disable. |
   *
   * _**Emitters**_
   *
   * A custom event will be emitted by the component when an action is clicked from the displayed menu.
   ...
   * | emitter | this component |
   * | cause | "useraction" |
   * | effect |  "move left", "move right", "merge left", "merge right", or "unmerge" |
   * | action | "click" |
   *
   * This component comes with a non-editable markup that is utilized when the component tag is added to the DOM.
   * The markup provided includes accessibility features.
   *
  ...
  class YAxisContext extends CIQ.UI.DialogContentTag {
    ...
    setContext(context) {
      const { stx } = context,
        ...
        yAxisRenderers = currentYAxis.renderers.filter((name) => {
          if (name === "_main_series") return stx.chart.symbol;
          const renderer = stx.chart.seriesRenderers[name];
          const hasSymbol = renderer.seriesParams.find(({ symbol }) => symbol);
          return hasSymbol;
        }),
        hasMerged = yAxisRenderers.length + studies.length >= 2;

      let menuItems = "";
      menuItems += hasAdjacentLeftAxis || isRHS ? this.menuItems.moveLeft : ``;
      menuItems +=
        hasAdjacentRightAxis || isLHS ? this.menuItems.moveRight : ``;
      ...
      const hasHistogramRenderer = currentYAxis.renderers.find((name) => {
        const renderer = stx.chart.seriesRenderers[name];
        const isHistogram = renderer.constructor === CIQ.Renderer.Histogram;
        return isHistogram;
      });

      // If the cq-allow-transform attribute is set to "false", the transform menu will not be displayed.
      if (
        this.getAttribute("cq-allow-transform") !== "false" &&
        !hasHistogramRenderer
      ) {
        menuItems += "<hr>";
        Object.keys(this.transformItems).forEach((item) => {
          menuItems += renderTransformItem(
            item,
            this.transformItems[item],
            item === currentYAxis.transformName,
          );
        });
      }
      ...
      // Focus the first menu item, for screen readers.
    ...
    /**
     * Handles yAxis actions as a result of the menu item chosen, either merging, moving or transforming the yAxis.
     *
     * @param {object} activator The object representing menuItem chosen.
     * @param {string} type Action type, e.g. "move", "merge", "unmerge" or "transform".
     * @param {string} option Parameter for the action, e.g. "left" or "right" Move or Merge direction or transform type.
     *
     * @tsmember WebComponents.YAxisContext
     */
    updateYAxis(activator, type, option) {
      if (type === "transform") {
        const setResult = setTransform(option, this.context.stx);
        const transformItems = this.querySelectorAll(".ciq-transform");
        transformItems.forEach((item) => {
          item.classList.remove("ciq-active");
          item.setAttribute("aria-checked", "false");
        });
        if (setResult) {
          activator.node.classList.add("ciq-active");
          activator.node.setAttribute("aria-checked", "true");
        }
      } else if (type === "unmerge") {
        unmergeYAxis(this.context.stx);
      } else {
        moveYAxis(this.context.stx, type, option);
      }
      this.emitCustomEvent({ effect: `${type} ${option}`.trim() });
      if (type !== "transform") this.close();
    }
  }

  function setTransform(type, stx) {
    const { currentYAxis } = stx;
    let { transformName } = currentYAxis;

    if (transformName === type) type = "";

    stx.setComparison(type, stx.chart, null, currentYAxis);
    stx.draw();
    if (currentYAxis === stx.chart.yAxis) {
      stx.layout.chartScale = type;
      stx.changeOccurred("layout");
    }
  ...
  YAxisContext.prototype.menuItems = {
    moveLeft: `<div stxtap="updateYAxis('move', 'left')" keyboard-selectable="true" role="menuitem" tabindex="-1">Move Left</div>`,
    moveRight: `<div stxtap="updateYAxis('move', 'right')" keyboard-selectable="true" role="menuitem" tabindex="-1">Move Right</div>`,
    mergeLeft: `<div stxtap="updateYAxis('merge', 'left')" keyboard-selectable="true" role="menuitem" tabindex="-1">Merge Left</div>`,
    mergeRight: `<div stxtap="updateYAxis('merge', 'right')" keyboard-selectable="true" role="menuitem" tabindex="-1">Merge Right</div>`,
    unmerge: `<div stxtap="updateYAxis('unmerge')" keyboard-selectable="true" role="menuitem" tabindex="-1">Unmerge</div>`,
  };

  YAxisContext.prototype.transformItems = {
    netchange: "Net Change",
    percent: "Percent Change",
  };

  function renderTransformItem(param, label, active) {
    return `
		<div stxtap="updateYAxis('transform', '${param}')" keyboard-selectable="true" class="ciq-switch-item ciq-transform ciq-${param} ${
      active ? "ciq-active" : ""
    }" role="menuitemcheckbox" aria-checked="${active ? "true" : "false"}">
			<span class="ciq-switch-label">${label}</span>
        ...
        if (this.hasAttribute("cq-keystroke-claim")) {
          ...
          for (let idx = 0; idx < toolsWithShortcuts.length; idx++) {
            ...
            if (/^[A-Za-z0-9]{1}$/g.test(letter)) {
              ...
              let buttonLabel =
                toolsWithShortcuts[idx].querySelector("[label]");
    ...
    class DrawingSettings extends Palette.classDefinition {
      ...
      setContext(context) {
        ...
        this.params = {
          ...
          showCalloutToggle: this.querySelector(
            "cq-show-callout .ciq-checkbox",
          ),
      ...
      toolSettings(activator, toolName) {
        ...
        const drawingParameters = CIQ.Drawing.getDrawingParameters(
          stx,
          toolName,
        );
        let actionEl;
       ...
       * @param {string} toolName Name of drawing tool.
       * @return Array of drawing setting element selectors
       *
       * @tsmember WebComponents.DrawingSettings
       */
      ...
      getColor(activator, mode) {
        ...
        if (specialColorStyles.includes("color-" + color)) {
          ...
          if (
            !color ||
            Math.abs(CIQ.hsl(bgColor)[2] - CIQ.hsl(color)[2]) < 0.2
          ) {
            const border = CIQ.chooseForegroundColor(bgColor);
            node.style.border = "solid " + border + " 1px";
          } else {
      ...
      setColor(color, mode) {
        ...
        this.getColor(
          {
            node: this.querySelector("cq-" + mode + "-color"),
          },
          mode,
        );
        this.emit();
      ...
      toggleCheckbox(activator, id, forceValue = null) {
        ...
        if (id === "showCallout" && cvp.vectorType === "trendline") {
          ["fontOptions", "fillColor"].forEach((name) =>
            this.params[name].classList[cvp[id] ? "add" : "remove"](
              "ciq-active",
            ),
          );
        }
...
export { __js_webcomponents_symbolInput_ as symbolInput };
...
if (typeof __TREE_SHAKE__ === "undefined" || !__TREE_SHAKE__) {
  _exports.CIQ.activateImports(
    ...
    __js_webcomponents_symbolInput_,
