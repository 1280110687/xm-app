...
CIQ.outerWidth = function (node) {
  log(
    "CIQ.outerWidth() has been deprecated.  Use `CIQ.elementDimensions(node, {margin:1, padding:1, border:1}).width` instead.",
  );
...
CIQ.findClosestParent = function (el, selector) {
  log(
    "CIQ.findClosestParent() has been deprecated.  Use `Element.parentElement.prototype.closest()` instead.",
  );
  var matchesFn;
  // find vendor prefix
  [
 ...
 * 					<span cq-translate-original="english">translation</span>
 * 							If it does not exist, a text node is returned.
 * @memberof CIQ
 * @deprecated As of 8.8.0. Use [CIQ.makeTranslatableElement(element, stx, original, language)]{@link CIQ.makeTranslatableElement}.
 */
CIQ.translatableTextNode = function (stx, english, language) {
  log(
    "CIQ.translatableTextNode() has been deprecated.  Use `CIQ.makeTranslatableElement(element, stx, original, language)`, where `element` is the parent element, instead.",
  );
  if (stx.translationCallback) {
    var translationNode = document.createElement("cq-translate");
    translationNode.setAttribute("cq-translate-original", english);
 ...
 * @deprecated Iterate through panels to delete instead. (See example below.)
 * @since 9.1.0 Deprecated
 * @memberof CIQ.ChartEngine
 * 
 * @example
...
CIQ.ChartEngine.prototype.deleteAllPanels = function () {
  log(
    "CIQ.ChartEngine.prototype.deleteAllPanels has been deprecated. Iterate through panels to delete instead.",
  );
...
CIQ.Studies.volumeChart = function (stx, sd, colorMap, borderMap) {
  ...
  var offset = 0;
  if (!borderMap) offset = (candleWidth * 0.65) / 2;
 ...
 *
 * @deprecated Use {@link CIQ.Marker.Performance#remove}.
 * @private
 * @since
 * - 7.1.0
...
CIQ.ChartEngine.prototype.removeDOMMarker = function (marker) {
  log(
    "CIQ.ChartEngine.prototype.removeDOMMarker has been deprecated. Please use CIQ.Marker.Performance.prototype.remove instead.",
  );
...
if (CIQ.UI.BaseComponent) {
   ...
   */
  CIQ.UI.BaseComponent.bind = function (node, params) {
    log(
      "CIQ.UI.BaseComponent.bind has been deprecated. Use CIQ.UI.BaseComponent.bindNode() instead.",
    );
    CIQ.UI.BaseComponent.bindNode(node, params);
  };
}
