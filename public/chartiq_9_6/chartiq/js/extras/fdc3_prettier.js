...
function registerFDC3(stx, config) {
  ...
  const checkAndReg = () => {
    ...
    const getRangePeriodicity = (range, isHolding) => {
      if (!range || range.type !== "fdc3.timeRange")
        return {
    ...
    const normalizeIndicator = (fdc3Indicator) => {
      ...
      if (type !== "chartiq.indicator" || !name) return;
      let ret = {};
      switch (name) {
        case "ma":
          ret = {
      ...
      if (custom) CIQ.extend(ret, custom);
          ...
          return () => {
            ...
            if (otherConfig) {
              for (const key in otherConfig) {
                const indicators = otherConfig[key],
                  myStx = stx;
                if (Array.isArray(indicators)) {
                  indicators.forEach((indicator) => {
                    addIndicator(myStx, indicator);
                  });
                } else {
                  addIndicator(stx, indicators);
                }
              }
            }
            (symbols || []).forEach((sym, s) => {
    ...
    const commonInstrumentContextListener = (isIntent) => {
      return (context) => {
        ...
        // take no further action if no symbols have been provided
