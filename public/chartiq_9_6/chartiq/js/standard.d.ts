...
declare module '../js/chartiq.js' {
  export namespace CIQ.ChartEngine {
    ...
    class Driver {
       ...
       * @param [behavior.loadingNewChart=false] READ ONLY boolean telling when a chart is loading
       * @param [behavior.updatingChart=false] READ ONLY boolean telling when a chart is updating
     ...
     * 		[getSavePreferences]CIQ.ChartEngine.getSavePreferences.
     * @param [multiChartId] The local storage identifier for multi chart
     * @static
     * @since
     * - 7.5.0
     * - 8.0.0 Renamed from `CIQ.UI.Chart#restorePreferences`.
     */
    function restorePreferences(stx: CIQ.ChartEngine, id?: string, multiChartId?: string): void
    /**
   ...
   * for real-data quotefeeds at [https://github.com/chartiq/quotefeeds]https://github.com/chartiq/quotefeeds.
   *
   */
  export namespace quotefeed {
     ...
     *
     * @example <caption>Setting <code>attribution</code> through the <code>dataCallback</code> object.
     *
     * <b>Note:</b> Using <code>dataCallback</code> to display a message requires that the attribution web component <em>not</em> be present in your template; otherwise the message will not display. For information on how to override the web component with custom attribution messages, see [https://documentation.chartiq.com/WebComponents.Attribution.html#messages.html#messages]https://documentation.chartiq.com/WebComponents.Attribution.html#messages.</caption>
     ...
     * @param [params.fetchMaximumBars]	-If set to true, the chart requires as much historical data as is available from the feed (params.ticks may also be set to 20,000 to set a safety max), regardless of start date. This is needed for some chart types since they aggregate data (for example, Kagi, Renko, or linebreak). Developers implementing fetch, should override params.tick and use a smaller number if their feed can't support that much data being sent back. The engine will then make multiple smaller calls to get enough data to fill the screen.
     ...
     * @param endDate The date of the last datum point currently available in the chart. You should return data from this point and then backward in time.
     * @param params						-Provides additional information on the data requested by the chart.
     * @param params.stx 			-The chart object requesting data
     * @param [params.symbolObject] 		-The symbol to fetch in object format; if a symbolObject is initialized ( see CIQ.ChartEngine#loadChart, CIQ.ChartEngine#addSeries, CIQ.Comparison.add )
     * @param params.period 				-The timeframe each returned object represents. For example, if using interval "minute", a period of 30 means your feed must return ticks (objects) with dates 30 minutes apart; where each tick represents the aggregated activity for that 30 minute period. **Note that this will not always be the same as the period set in CIQ.ChartEngine#setPeriodicity, since it represents the aggregation of the raw data to be returned by the feed server, rather than the final data to be displayed.**
  ...
  export namespace CIQ {
    ...
    class Drawing {
       ...
       * @param  forceClear Indicates that the user explicitly deleted the drawing (advanced usage).
       */
       ...
       * @param  stx  The chart object.
       * @param  [field] Field in the dataset, if something other than default.
       * @return  Whether a drawing can be made on the field.
       * @since 8.6.0
       */
      public isAllowed(stx: CIQ.ChartEngine, field?: string): boolean
      /**
       ...
       * @param [ignoreHighlight] True to ignore highlighting status when determining color (for example, if a label, or if determining fill color).
       ...
       */
      public getLineColor(color: string, ignoreHighlight?: boolean): string
    }
    /**
    ...
    class EaseMachine {
       ...
       * - d = duration
       *
       * @param fc The cubic function or the function name of a static method.
       ...
       * @param [ownerWindow] The window where the parent chart is loaded.
       ...
       * @since 8.9.1 Added `ownerWindow` parameter.
       */
      public stop(ownerWindow?: object): Record<string, number>
    }
    /**
    ...
    class Marker {
       ...
       * @param [params.candleOffset] Number of candlewidths to offset the marker left or right from its horizontal position.
       ...
       * @param [params.field] When `yPositioner` is set to position relative to a candle, this parameter determined which
       ...
       * @param [params.groupable=true] When multiple groupable markers are placed at the same position, they will consolidate
       ...
       *
       * @return Marker representing the group
       *
       * @since 9.2.0
       */
      public getMarkerGroup(): CIQ.Marker.Group
      /**
       ...
       *
       * @param [dropzone=true] The container which will receive the dragged marker when it is dropped.  Defaults to `true`,
       * 	which will use the marker's panel for the dropzone.  Setting this parameter to false will disable dragging.
       *
       * @since 9.1.0
       */
      public makeDraggable(dropzone?: HTMLElement|boolean): void
      /**
    ...
    class Market {
       ...
       * @param [parms.inZone] A valid time zone from the timeZoneData.js library. This should represent the time zone for any input dates such as `parms.begin` in this function or `parms.end` in CIQ.Market.Iterator#futureTick. Defaults to browser time zone if none set.  - If no market zone is present it will assume browser time.
       * @param [parms.outZone] A valid time zone from the timeZoneData.js library. This should represent the time zone for the returned dates. Defaults to browser time zone if none set.  - If no market zone is present it will assume browser time.
    ...
    class Visualization {
       ...
       * @param [attributes.document] Optional document where visualization should be rendered.
       * @example
       * let svg=new CIQ.Visualization({ renderFunction: CIQ.SVGChart.renderPieChart });
       * svg.updateData({"Low":{name:"low", value:30}, "High":{name:"high", value:70}});
       * @since
       ...
       * of key/value pairs.
       *
       * @param arg1 An attribute key or an object of attribute key/value pairs.
    ...
    interface ChartEngine {
       ...
       *
       * **Note:** By default, if negative values are present, the histogram will not display bars pointing downwards. Instead, the baseline of the histogram will be adjusted to the minimum value detected, and all bars will rise upward from that point.
       * To allow for bars to draw downwards from a particular point, the histogram's  y-axis must contain a **baseline value.**
       *
       ...
       * @param  [params.bindToYAxis=false] Set to true to bind the histogram to its y-axis; otherwise, it will overlay and not move with the axis.
       ...
       * @param  [param.colMargin] Space between columns in the cluster. Larger or equal to 1 defines pixel width between clustered columns, if less than 1 - percentage of the cluster width.
       * @param  [params.borders=true] Histogram bar border override. Set to 'false' to stop drawing borders, even if seriesParams.border_color_X are set.
       ...
       * @param  [seriesParams.clusterPriority] Priority of series rendering. Series with higher values are rendered first.
       *
       * **Parameters:**
       * <table>
       ...
       * - 9.6.0 Added `seriesParams.clusterPriority` and `param.colMargin` parameter.
       */
      drawHistogram(
        params: {
          name?: string,
       ...
       * where appropriate.
       * @param htmlElement The HTML element for the chart container.
       * @example
       ...
       * @return Returns true if the drawing was successfully activated. Returns false if inactivated or unsuccessful.
       * @since
       * - 3.0.0
       ...
       * @param forceText Skip the context menu and begin text edit. Used on touch devices.
       * @since
       * - 6.2.0
       * - 8.6.0 Add `forceText` flag.
       */
      rightClickDrawing(drawing: CIQ.Drawing, forceEdit: boolean, forceText: boolean): void
      /**
       * INJECTABLE
       *
       ...
       * stxx.clearDrawings()
       */
      changeVectorType(value: string|null): void
      /**
       ...
       * @return  Returns `true` if property was assigned.
       * @example
       ...
       *
       * @param id Event id to add
       * @param params Parameters to pass through to addSeries.  See CIQ.ChartEngine#addSeries for details.
       ...
       * @since 9.0.0
       */
      addEvent(id: string, params: object, cb?: Function): void
      /**
       ...
       *
       * @param stx The chart object associated with the event.
       * @param error An error message, if one occurred.
       * @param series The event's series object.
       * @param data An array of quotes in required JSON format, if no error occurred.
       *
       * @since 9.0.0
       */
      processEventResults(
       ...
       * @since 9.0.0
       */
      takedownEventResults(stx: CIQ.ChartEngine, id: string): void
      /**
       ...
       * translating all of the text for the given language. See CIQ.I18N.translateUI.
       *
       ...
       * CIQ.makeTranslatableElement or CIQ.ChartEngine#translateIf.
       *
       ...
       * @since 8.5.0
       */
      translateUI(root?: HTMLElement): void
      /**
       ...
       * 		touch devices.
       * @param forceText Skip the context menu and begin text edit immediately. Usually for
       * 		touch devices.
       *
       * @since
      ...
      /**
       * Allows the chart's HTML controls (zoom, home, etc) to be draggable up and down.
       * Useful to reveal blocked elements in the chart.
       *
       * @since 8.6.0
       */
      allowDragControls(): void
      /**
       ...
       * @param [behavior.prefetchAction] Optional callback that allows you to add a custom
       ...
       * For example, to add a step line, you would select a [Lines]CIQ.Renderer.Lines renderer, and then set its `step` attribute, right through the addSeries API call.
       * ```
       * stxx.addSeries(
       * 		"SNE",
       * 		{
       ...
       *
       * **Note:** Stacked histograms should _not_ be used with a combination of negative and positive values, nor should they be used with a baseline.
       *
       ...
       * @param [parameters.renderer=CIQ.Renderer.Lines] Rendering Set to the desired [renderer]CIQ.Renderer for the series.
       * - If not set, defaults to [Lines]CIQ.Renderer.Lines when `color` is set.
       * - Not needed for hidden series.
       * @param [parameters.name] Rendering Set to specify renderer's name.  Otherwise, `id` will be used.
       * @param [parameters.display=id/symbol] Rendering Set to the text to display on the legend. If not set, the id of the series will be used (usually symbol).  If id was not provided, will default to symbol.
       * @param [parameters.symbol=id] Data Loading The symbol to fetch in string format. This will be sent into the fetch() function, if no data is provided.  If no symbol is provided, series will use the `id` as the symbol. If both `symbol` and `symbolObject` are set, `symbolObject` will be used.
       * @param [parameters.symbolObject=id] Data Loading The symbol to fetch in object format. This will be sent into the fetch() function, if no data is provided. If no symbolObject is provided, series will use the `id` as the symbol. You can send anything you want in the symbol object, but you must always include at least a 'symbol' element. If both `symbol` and `symbolObject` are set, `symbolObject` will be used.
       * @param [parameters.field=Close/Value] Data Loading Specify an alternative field to draw data from (other than the Close/Value). Must be present in your pushed data objects or returned from the quoteFeed.
       ...
       * @param [parameters.shareYAxis=false] Rendering
       ...
       * @param [parameters.marginTop=0] Rendering Percentage (if less than 1) or pixels (if greater than 1) from top of panel to set the top margin for the series.<BR>**Note:** this parameter is to be used on **subsequent** series rendered on the same axis. To set margins for the first series, CIQ.ChartEngine.YAxis#initialMarginTop needs to be used.<BR>**Note:** not applicable if shareYAxis is set.
       * @param [parameters.marginBottom=0] Rendering Percentage (if less than 1) or pixels (if greater than 1) from the bottom of panel to set the bottom margin for the series.<BR>**Note:** this parameter is to be used on **subsequent** series rendered on the same axis. To set margins for the first series, CIQ.ChartEngine.YAxis#initialMarginBottom needs to be used.<BR>**Note:** not applicable if shareYAxis is set.
       * @param [parameters.width=1] Rendering Width of line in pixels
       * @param [parameters.minimum] Rendering Minimum value for the series. Overrides CIQ.minMax result.
       * @param [parameters.maximum] Rendering Maximum value for the series. Overrides CIQ.minMax result.
       * @param [parameters.color] Rendering Color used to draw the series line. Causes the line to immediately render an overlay. Only applicable for default or single-color renderers.
       ...
       * @param [parameters.baseColor=parameters.color] Rendering Color for the base of a mountain series.
       * @param [parameters.pattern='solid'] Rendering Pattern to draw line, array elements are pixels on and off, or a string e.g. "solid", "dotted", "dashed"
       * @param [parameters.fillGaps] Data Loading If CIQ.ChartEngine#cleanupGaps is enabled to clean gaps (not 'false'), you can use this parameter to override the global setting for this series.
       ...
       * @param [parameters.gapDisplayStyle=true] Rendering Defines how (or if) to **render** (style) connecting lines where there are gaps in the data (missing data points), or isolated datapoints.
       ...
       * @param [parameters.fillStyle] Rendering Fill style for mountain chart (if selected). For semi-opaque use rgba(R,G,B,.1).  If not provided a gradient is created with color and baseColor.
       * @param [parameters.permanent=false] Rendering Set to `true` to activate. Makes series unremoveable by a user **when attached to the default renderer**. If explicitly linked to a renderer, see CIQ.Renderer#attachSeries for details on how to prevent an attached series from being removed by a user.
       * @param [parameters.data] Data Loading Data source for the series.
       ...
       * @param [parameters.panel] Rendering The panel name on which the series should display. If the panel doesn't exist, one will be created. If `true` is passed, a new panel will also be created.
       * @param [parameters.action='add-series'] Rendering Overrides what action is sent in symbolChange events. Set to null to prevent a symbolChange event.
       * @param [parameters.loadData=true] Data Loading Include and set to false if you know the initial data is already in the masterData array or will be loaded by another method. The series will be added but no data requested. Note that if you remove this series, the data points linked to it will also be removed which may create issues if required by the chart. If that is the case, you will need to manually remove from the renderer linked to it instead of the underlying series itself.
       * @param [parameters.extendToEndOfDataSet] Rendering Set to true to plot any gap at the front of the chart.  Automatically done for step charts (set to false to disable) or if parameters.gapDisplayStyle are set (see CIQ.ChartEngine#addSeries)
       * @param [parameters.displayFloatingLabel=false] Rendering Set to false to disable the display of a y-axisfloating label for this series.
       * @param [parameters.baseline] Rendering If a boolean value, indicates whether the series renderer draws a baseline. If an object, must be the equivalent of CIQ.ChartEngine.Chart#baseline.
       * @param [parameters.responseHandler] Optional function to override the processing of data. Function accepts an object argument with `symbol`and `symbolObject` properties.  If omitted, uses a default implementation.
       * @param [parameters.retoggle] Set to true to indicate that series should be reloaded when loading a new chart.
       * @param [parameters.processResults] Optional function to perform on the returned result set. Function accepts arguments (stx, error, series, data). Called within responseHandler function.
       * @param [parameters.takedownResults] Optional function to perform when removing series. Complements `processResults`. Function accepts arguments (stx, seriesId).
       * @param [parameters.stretchGaps] Set to true to cause gaps to be represented by a horizontal line.
       * @param [parameters.noStorage] Set to true to disable export of the series when saving layout.
       * @param [parameters.periodicParams] Set to indicate that quote feed supplied data is periodic and needs to be expanded for given range.
       * @param [parameters.periodicParams.endOfPeriod] Set to indicate periodic data Date property is at the end of period insted of default beginning.
       * @param [parameters.periodicParams.period="quarter"] Span period defaults to quarter.
       ...
       * - 9.6.0 Added `parameters.periodicParams`, `parameters.periodicParams.endOfPeriod`, `parameters.periodicParams.period`, `parameters.labels`, `parameters.labels.backgroundColor`, `parameters.labels.textColor`, `parameters.labels.font`, `parameters.labels.bold`, `parameters.labels.italic`, `parameters.labels.padding`, `parameters.labels.verticalOffset`, `parameters.labels.insideBar`, `parameters.labels.lastVisibleOnly`
       *
       ...
       * @param [yaxis] The y-axis to apply the comparison to. If not provided, the chart.yAxis is used.
       * @since
       ...
       * 	 - "netchange"
       * 		> The "netchange" scale calculates the change between the first visible bar to the last visible bar.
       * 		> This is useful when you want to see the actual change in price, rather than the price itself.
       * 		> See CIQ.Comparison.priceToNetChange for details on how to set basis for "netchange" scale.
       * 	 - "percent"
       ...
       *
       * @param [params] Optional params object. See CIQ.ChartEngine#draw for details.
       *
       * @since 8.9.3
       */
      drawWithRange(params?: Object): void
      /**
       ...
       * >   stxx.layout.range = {
       * >     padding: stxx.preferences.whitespace,
       * >     dtLeft,
       * >     dtRight,
       * >     periodicity: {
       ...
       * @param [params.field] Optionally set to a series field which has OHLC data stored beneath it in the dataSegment
       * @param  [params.highlight] Set to true to indicate plot is highlighted.
       * @param [params.isHistogram] Set to true to specify a histogram. Doing so will prevent horizontal shadows.
       * @param [params.yAxis=panel.yAxis] Set to specify an alternate y-axis
       ...
       * @param  drawing The drawing to activate. Send null to deactivate.
       * @since 3.0.0
       */
       ...
       * @param  [arrSubset] Optional subset of markers to move to new panel
       * @since
       * - 2016-07-16
       ...
       *  the high/low range. For those chart types, this function will return the actual high/low to be used by the marker placement function. This is only valid when CIQ.Renderer#highLowBars is true.
       *
       ...
       * @param [ignoreLayout] If true don't allow chart type to dictate high and low.
       ...
       */
      getBarBounds(quote: object, ignoreLayout?: boolean): object
      /**
       ...
       * @param params.stx The chart engine
       ...
       * @param [params.forceLoad] Forces a complete load (used by loadChart). Supersedes `useExistingData`.
       * @param [params.useExistingData] Set to true to indicate existing data is sufficient.
       ...
       * @param [params.useExistingData] Set to true to indicate existing data is sufficient.
       * @param [params.chart] Which chart, defaults to "chart"
       * @param [params.periodicity] The granularity of the chart, comprising its period and interval.
       * @param [cb] Optional callback
       * @example
       ...
       *
       * @param params Parameters
       * @param [params.type] Filter for only studies of this type
       * @param [params.name] Filter for only studies that have this name
       * @param [params.output] Filter for only studies that contain this output name in the output map
       * @return Array of study descriptors
       *
       * @since 9.1.0
       */
      getStudies(params: {type?: string, name?: object, output?: object}): any[]
    }
    /**
     ...
     *
     * There are more timezones loaded in the chart by default from **\/examples/markets/timezones.js**.
     * 		You can get a full list by running `timezoneJS.timezone.getAllZones();` from your browser
     * 		console. (`timezoneJS` can be made available by importing it from **standard.js** and
     * 		assigning it to the global window object.)
     *
     * Feel free to add other timezones and rules to this map if you want them to be available to
     * 		users. Our complete list can be downloaded from
     * 		[here](https://download.chartiq.com/timeZones/timezoneDataObject.txt).
     * 		This file is large, so add timezones with discretion.
     *
     * Although we do update this file periodically, it may not be available immediately after every
     * 		timezone change. If you require immediate updates, you should subscribe to a
     * 		notification system that alerts you of these changes, then adjust the file as needed.
     * 		<a href="https://www.iana.org/time-zones" target="blank" referrer="none">IANA</a> is a
     * 		good source.
     *
     * The following code snippet demonstrates how to add timezones and rules to this object. (You
     * 		can add synonyms this way as well). To save space, you may want to cherry pick
     * 		the zones that you will need and then add them in your initialization code.
     * ```
  ...
  export namespace CIQ.Drawing {
    ...
    class annotation {
       ...
       * @param [obj.prm] Whether the drawing is permanent
       * @param [obj.hdn] Whether the drawing is hidden
    ...
    class shape {
       ...
       * @param [obj.fc] The fill color.
       * @param [obj.pnl] The panel name.
       * @param [obj.ptrn] Pattern for line "solid", "dotted", or "dashed". Defaults to solid.
       ...
       * @param [obj.fld] Field to which drawing is associated.
       * @param [obj.ver] Version number of the shape.
       * @param [obj.prm] Whether the drawing is permanent
       * @param [obj.hdn] Whether the drawing is hidden
       * @since 8.4.0 Added `fld` property to `obj` parameter.
       * @since 9.0.0 Added `prm` and `hdn` property to `obj` parameter.
     ...
     *
     * Extends CIQ.Drawing.segment and renders an optional CIQ.Drawing.callout
     ...
     * @param  excludeList Exclusion list of tools in object form; for example, {"vertical":true,"annotation":true}.
     * @return Map of tool names and types.
     * @since 3.0.0
     */
     ...
     * @param stx The chart object.
     * @param drawingName Name of drawing; for example: "ray", "segment".
     * @return Map of parameters used in the drawing type, with their current values.
     * @since 3.0.0
     */
     ...
     * @param stx The chart object.
     * @param toolName Name of drawing tool; for example: "ray", "segment", "fibonacci".
     * @since 6.0.0
     */
     ...
     *
     * @param stx The chart object.
     * @param toolName Name of active drawing tool; for example: "ray", "segment", "fibonacci".
  ...
  export namespace CIQ.Marker {
    ...
    class Simple {
       ...
       * @param [params.pinnable] Allow marker to be "pinned". Requires **js/extras/pinnedMarkers.js,** which is part of the ChartIQ Extras Package. This flag is false by default. See CIQ.Marker.prototype.makePinnable for instructions on using this flag.
       * @param [params.id] Required when `params.pinnable` is set to `true`. A unique identifier for the marker. This allows the pinned marker position to persist over a browser refresh.
       * @since
       * - 9.1.0 Added `pinnable` parameter, which makes all new markers pinnable. This feature requires **js/extras/pinnedMarkers.js,** which is part of the ChartIQ Extras Package.
       *
       * @example
       ...
       *
       * @param marker The marker to which this node belongs.
       *
       * @since 9.1.0
       */
      public remove(marker: CIQ.Marker): void
    }
    /**
    ...
    class Group {
       ...
       * @param params Configuration parameters.
       * @param params.stx The chart engine.
       * @param [params.groupKey] The shared groupKey value for markers associated with this group
       * @param params.x A valid date, tick, or bar (depending on the selected `xPositioner`) used to select a candle to
       * 					which the marker is associated.
       * @param [params.y] A valid value for positioning the marker on the y-axis (depending on selected `yPositioner`).
       * 					If this value is not provided, the marker is set "above_candle" as long as a valid candle is selected
       ...
       * @param [params.noInteraction] Set to `true` to disable the marker's default click handler.
       *
       * @since
       * - 9.2.0
       * - 9.3.0 Add `noInteraction` parameter.
       ...
       * Returns the CIQ.GroupMarker object representing the marker if it is placed in a group.
       *
       * @return Array of `CIQ.Marker` objects associated with this group
       *
       * @since 9.2.0
       */
      public getMarkersInGroup(): CIQ.Marker[]
      /**
       ...
       * @since 9.2.0
       */
      public createNode(params: Object): void
      /**
    ...
    class GroupNode {
       ...
       *
       * @param params Parameters to describe the marker
       * @param params.groupMap Reference to the stxx.markerHelper.groupMap property associate with marker group.
       * @param [params.displayStem=true] Set to false to draw the marker at a specific point and not include the stem.
       * @param params.type The marker type to be drawn.
       * Available options are:
       * - "circle"
       * - "square"
       ...
       *
       * @param e Marker interaction event handler
       *
       * @since 9.2.0
       */
       ...
       *
       * @return A valid css color value.
       *
       * @since 9.3.0
       *
       * @example
       ...
       * };
       */
      public getGroupMarkerBackgroundColor(): string
      /**
       ...
       *
       * @return A negative value indicates that a should come before b. A positive value indicates that a should come after b. Zero or NaN indicates that a and b are considered equal.
       *
       * @since 9.3.0
       *
       * @example
       ...
       *
       * @param marker The marker within the group with x positioner to display
       * @return Formatted date string.
       *
       * @since 9.3.0
       */
      public getDateLabel(marker: CIQ.Marker): string
      /**
       * Displays a marker expansion for the group marker.  If the expansion is already displayed, it is removed.
       *
       * @param marker The marker to display or hide the expansion.
       * @param e Marker interaction event handler
       *
       * @since 9.3.0
       */
      public handleMenuClick(marker: CIQ.Marker, e: Event): void
      /**
       * Set visibility style of the group expansion holder based on whether it is empty or not
       *
       * @since 9.2.0
       */
      public setExpansionHolderVisibility(): void
    }
    /**
     ...
     * @param [evenIfNotVisible] True to position the marker even if not visible. This makes any transition
     ...
     */
    function positionContentVerticalAndHorizontal(node: HTMLElement, evenIfNotVisible?: boolean): void
  }
  ...
  export namespace CIQ.Studies {
    ...
    class StudyDescriptor {
       ...
       */
      public inputs: Record<string, (string | number | boolean)>
      /**
       * Keys for each possible study output with its corresponding rendering color.
       */
      public outputs: Record<string, string | number>
      /**
       * Keys for each of the study's possible plot parameters.
       */
      public parameters: Record<string, string | number | boolean | Record<string, string | number | boolean>>
      /**
       ...
       */
      public outputMap: Record<string, string | number>
      /**
       ...
       * @since 8.7.0
       */
      public toggleDisabledState(stx: CIQ.ChartEngine): void
      /**
     ...
     *
     */
    let impliedVolatilityAvailable: boolean
    /**
     ...
     * @since 9.0.0
     */
    let useTranslationDelimiter: boolean
    /**
     ...
     * @static
     * @deprecated use CIQ.ChartEngine.getQuoteFieldValue instead
     * @since
     * - 8.9.1
     * - 9.4.0 Deprecated, use CIQ.ChartEngine.getQuoteFieldValue instead
     */
     ...
     * @return Map of available entries from CIQ.Studies.studyLibrary.
     * @since 3.0.0
     */
     ...
     *
     * @param studyOutputAliasList The alias object with a key being the study name and the value being an object that maps what you want the pre-existing study output to be.
     * @example
     * CIQ.Studies.assignAliasesToStudies({"PMO": { "PMOSignal": "Signal" }});
     * @since 9.1.0
     */
    function assignAliasesToStudies(studyOutputAliasList: object): void
    /**
  ...
  export namespace CIQ.ChartEngine.Driver {
     ...
     *
     * Provides back-end search functionality for the [cq-lookup]WebComponents.Lookup
     * web component.
     *
    ...
    class Lookup {
       ...
       * 		[cq-lookup]WebComponents.Lookup web component's input field.
       * 		<p>**Note:** This parameter is overridden by the `cq-exchanges` attribute of
       * 		[cq-lookup]WebComponents.Lookup.
       *
       * @since 6.0.0
       *
       * @see CIQ.UI.Context#setLookupDriver
       ...
       * @param [filter] Text indicating security type for the lookup server to filter.
   ...
   * @since 8.6.0 Allow the static function name to be passed instead of the function itself.
   */
  export namespace CIQ.EaseMachine {
    /**
     * Quadratic easing out - decelerating to zero velocity (https://gizma.com/easing/).
     *
     * @param t Current time (t should move from zero to d).
     * @param b Starting value.
     * @param c Change in value (b + c = ending value ).
     * @param d Duration.
     *
     * @since 8.6.0
     */
    function easeInOutQuad(t: number, b: number, c: number, d: number): void
    /**
     ...
     * @since 8.6.0
     */
    function easeInOutCubic(t: number, b: number, c: number, d: number): void
    /**
     ...
     * @since 8.6.0
     */
    function easeOutCubic(t: number, b: number, c: number, d: number): void
  }
  ...
  export namespace CIQ.I18N {
     ...
     * or ensure your code explicitly translates the new content using CIQ.makeTranslatableElement or CIQ.ChartEngine#translateIf.
     *
  ...
  export namespace CIQ.Comparison {
    ...
    /**
     * Transform function for net change charting
     *
     * @param  stx	  The charting object
    ...
    /**
     * Untransform function for net change charting
     * @param  stx	  The charting object
     * @param  chart	The specific chart
     * @param  netChange The price to untransform
     * @param  [yAxis] Axis where price is to be found
     * @param  [field] Field whose price needs converting, otherwise the main series is converted
  ...
  export namespace CIQ.Share {
     ...
     * @param    [params.imageType]   Specifies the file format your image will be output in. The default is PNG and the format must be supported by your browser.
     ...
     * @since 9.0.0
     */
    function saveChartLayout(stx: CIQ.ChartEngine): Promise<string>
    /**
     ...
     * @since 9.0.0
     */
    function onClipboard(cb: Function): void
    /**
     ...
     * @since 9.0.0
     */
    function loadChartFromID(stx: CIQ.ChartEngine, shareID: string): Promise<void>
    /**
     * Loads a chart with the given configuration.
     *
     * @param chart Chart Object
     * @param configuration  Share configuration
     * @return
     * @async
     * @since 9.0.0
     */
    function loadChartFromLayout(chart: object, configuration: string): Promise<void>
  }
}
export function createEngine(_export: object): void
export function customCharts(_export: object): void
export function drawing(_export: object): void
export function easeMachine(_export: object): void
export function equations(_export: object): void
export function events(_export: object): void
export function i18n(_export: object): void
export function interaction(_export: object): void
export function markers(_export: object): void
export function market(_export: object): void
export function movement(_export: object): void
export function nameValueStore(_export: object): void
export function quoteFeed(_export: object): void
export function series(_export: object): void
export function share(_export: object): void
export function span(_export: object): void
export function storage(_export: object): void
export function studies(_export: object): void
export function symbolLookupBase(_export: object): void
export function theme(_export: object): void
export function timezone(_export: object): void
export function touch(_export: object): void
export function visualization(_export: object): void
export function medianPrice(_export: object): void
export function momentum(_export: object): void
export function priceRelative(_export: object): void
export function vwap(_export: object): void
export function zigzag(_export: object): void