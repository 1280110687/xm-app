...
import CopyPlugin from "copy-webpack-plugin"; // copies 3rd party libraries loaded on demand such as html2canvas
...
const resolvedPaths = (subdir = "") =>
  isNpm
    ? [
      path.join(chartiqDir, "technical-analysis", subdir),
      path.join(coreDir, subdir),
      path.join(chartiqDir, "component-ui", subdir),
      path.join(chartiqDir, "web-components", subdir),
      path.join(chartiqDir, "active-trader", subdir),
      path.join(chartiqDir, "command-line", subdir),
      path.join(chartiqDir, "fts-chart-explainer", subdir),
      path.join(chartiqDir, "gonogo", subdir),
      path.join(chartiqDir, "institutional", subdir),
      path.join(chartiqDir, "multi-periodicity", subdir),
      path.join(chartiqDir, "scriptiq", subdir),
      path.join(chartiqDir, "trading-central", subdir),
      path.join(chartiqDir, "visual-earnings", subdir)
    ]
    : path.join(chartiqDir, subdir);
...
export default {
  entry: {
    core: "./src/sample-template-webpack.js"
  },
  mode: env,
  module: {
    /**
    ...
    splitChunks: {
      ...
      cacheGroups: {
        ...
        thirdparty: {
          name: "thirdparty",
          priority: 30,
          test: /[\\/]node_modules[\\/](!chartiq)[\\/]|[\\/]thirdparty[\\/]/
        },
        examples: {
          name: "examples",
          priority: 40,
  ...
  output: {
    path: path.resolve(__dirname, 'h5-dist'), // 输出文件存放的目录
  ...
  plugins: [
    ...
    new CopyPlugin({
      patterns: [
        {
          from: path.join(coreDir, "js/thirdparty/html2canvas.min.js"),
    ...
    new HtmlWebpackPartialsPlugin({
      path: path.join(
        ...
        "templates",
        "partials",
        "sample-template-advanced-context.html"
      ),
      location: "cq-context"
    }),
    /**
